...
embedding을 통해 문서를 비교하는 것은 여러 언어에 걸쳐 작동한다는 이점이 있습니다. "Harrison says hello"와 "Harrison dice hola"는 의미론적으로 동일한 의미를 가지고 있기 때문에 vector space에서 유사한 위치를 차지합니다.

그러나 문서를 벡터화하기 전에 LLM을 사용하여 **문서를 다른 언어로 번역**하는 것이 여전히 유용할 수 있습니다. 이는 특히 사용자가 다양한 언어로 knowledge base를 쿼리할 것으로 예상되거나, 특정 언어에 대해 최신 embedding model을 사용할 수 없는 경우에 유용합니다.

OpenAI의 function calling 기능을 사용하여 언어 간 문서를 번역하는 [Doctran](https://github.com/psychic-api/doctran) 라이브러리를 사용하여 이를 수행할 수 있습니다.
...
## Async 버전을 사용한 Output

문서를 번역한 후, 결과는 page_content가 대상 언어로 번역된 새 문서로 반환됩니다. async 버전은 문서가 여러 부분으로 청크될 때 성능을 향상시킵니다. 또한 올바른 순서로 output을 반환하도록 보장합니다.
