...
이 json splitter는 chunk 크기를 제어하면서 json 데이터를 [분할](/oss/integrations/splitters/)합니다. json 데이터를 깊이 우선으로 순회하며 더 작은 json chunk를 생성합니다. 중첩된 json 객체를 온전하게 유지하려고 시도하지만, chunk를 min_chunk_size와 max_chunk_size 사이로 유지하기 위해 필요한 경우 분할합니다.
