...
검색 시 `(document, score)` tuple 목록과 함께 similarity score를 반환하도록 할 수도 있습니다. 여기서 `document`는 텍스트 내용과 metadata를 포함하는 LangChain @[`Document`] object입니다.
...
- [LangChain로 RAG 앱 만들기](/oss/langchain/rag)
