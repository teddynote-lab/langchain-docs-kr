...
[ManticoreSearch](https://manticoresearch.com/)는 빠르고 확장 가능하며 사용자 친화적인 기능을 제공하는 오픈 소스 검색 엔진입니다. [Sphinx Search](http://sphinxsearch.com/)의 포크로 시작하여 현대적인 검색 엔진 기능과 개선 사항을 통합하도록 발전했습니다. ManticoreSearch는 강력한 성능과 다양한 애플리케이션에 쉽게 통합할 수 있다는 점에서 차별화됩니다.

ManticoreSearch는 최근 검색 엔진 버전 6.2부터 [벡터 검색 기능](https://manual.manticoresearch.com/dev/Searching/KNN)을 도입했으며, [manticore-columnar-lib](https://github.com/manticoresoftware/columnar) 패키지가 설치된 경우에만 사용할 수 있습니다. 이 기능은 벡터 유사도를 기반으로 검색을 수행할 수 있게 해주는 상당한 발전입니다.

현재 벡터 검색 기능은 검색 엔진의 개발(dev) 버전에서만 사용할 수 있습니다. 따라서 이 기능을 효과적으로 활용하려면 개발용 [manticoresearch-dev](https://pypi.org/project/manticoresearch-dev/) Python 클라이언트를 사용해야 합니다.

## 환경 설정

ManticoreSearch로 Docker 컨테이너 시작 및 manticore-columnar-lib 패키지 설치 (선택 사항)
