...
`SingleStoreVectorStore`를 초기화하려면 @[`Embeddings`] 객체와 SingleStore 데이터베이스에 대한 연결 매개변수가 필요합니다.
