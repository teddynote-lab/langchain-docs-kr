...
>[Etherscan](https://docs.etherscan.io/)은 탈중앙화 스마트 컨트랙트 플랫폼인 Ethereum을 위한 선도적인 블록체인 탐색기, 검색, API 및 분석 플랫폼입니다.

## Overview

`Etherscan` loader는 `Etherscan API`를 사용하여 `Ethereum Mainnet`의 특정 계정에 대한 거래 내역을 로드합니다.

진행하려면 `Etherscan api key`가 필요합니다. 무료 api key는 초당 5회 호출 할당량을 가집니다.

이 loader는 다음 6가지 기능을 지원합니다:

* Ethereum Mainnet의 특정 계정에 대한 일반 거래 조회
* Ethereum Mainnet의 특정 계정에 대한 내부 거래 조회
* Ethereum Mainnet의 특정 계정에 대한 erc20 거래 조회
* Ethereum Mainnet의 특정 계정에 대한 erc721 거래 조회
* Ethereum Mainnet의 특정 계정에 대한 erc1155 거래 조회
* Ethereum Mainnet의 특정 계정에 대한 wei 단위의 ethereum 잔액 조회

계정에 해당 거래가 없는 경우, loader는 하나의 document를 포함하는 list를 반환합니다. document의 내용은 ''입니다.

위에서 언급한 다양한 기능에 접근하기 위해 loader에 다른 filter를 전달할 수 있습니다:
...
* "erc721_transaction"
* "erc1155_transaction"
filter는 기본적으로 normal_transaction으로 설정됩니다

질문이 있으시면 [Etherscan API Doc](https://etherscan.io/tx/0x0ffa32c787b1398f44303f731cb06678e086e4f82ce07cebf75e99bb7c079c77)에 접속하거나 [i@inevitable.tech](mailto:i@inevitable.tech)로 연락주시기 바랍니다.

거래 내역과 관련된 모든 함수는 Etherscan 제한으로 인해 최대 1000개의 내역으로 제한됩니다. 필요한 거래 내역을 찾기 위해 다음 매개변수를 사용할 수 있습니다:

* offset: 기본값 20. 한 번에 20개의 거래를 표시합니다
* page: 기본값 1. 페이지네이션을 제어합니다.
* start_block: 기본값 0. 거래 내역이 0 블록부터 시작됩니다.
* end_block: 기본값 99999999. 거래 내역이 99999999 블록부터 시작됩니다
* sort: "desc" 또는 "asc". 최신 거래를 가져오기 위해 기본값은 "desc"로 설정됩니다.
...
## 사용자 정의 매개변수를 사용한 normal transaction loader 생성하기
