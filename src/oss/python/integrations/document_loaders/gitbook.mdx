...
>[GitBook](https://docs.gitbook.com/)은 팀이 제품부터 내부 지식 베이스 및 API에 이르기까지 모든 것을 문서화할 수 있는 현대적인 문서화 플랫폼입니다.
...
이 기능이 작동하려면 GitbookLoader가 루트 경로(이 예제에서는 `https://docs.gitbook.com`)로 초기화되어야 하며 `load_all_paths`가 `True`로 설정되어야 합니다.
