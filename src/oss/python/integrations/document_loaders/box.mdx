...
`BoxLoader` 클래스는 Box의 비정형 콘텐츠를 LangChain의 @[`Document`] 형식으로 가져오는 데 도움을 줍니다. Box file ID를 포함하는 `List[str]` 또는 Box folder ID를 포함하는 `str`을 사용하여 이 작업을 수행할 수 있습니다.
...
connector는 이 필드를 integration이 작동하는 데 필요한 필드 목록에 추가한 다음 `"metadata" : { "source" : "source, "shared_link" : "shared_link" }`와 같이 @[`Document`] 또는 `Blob`에서 반환된 metadata에 결과를 추가합니다. 해당 파일에 대해 필드를 사용할 수 없는 경우 `"shared_link" : ""`와 같이 빈 문자열로 반환됩니다.
