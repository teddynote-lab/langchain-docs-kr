...
loader는 "single"과 "elements" 두 가지 모드 중 하나로 실행할 수 있습니다. "single" 모드를 사용하면 document가 단일 @[`Document`] object로 반환됩니다. "elements" 모드를 사용하면 unstructured library가 document를 `Title` 및 `NarrativeText`와 같은 요소로 분할합니다. mode 이후에 추가 `unstructured` kwargs를 전달하여 다양한 `unstructured` 설정을 적용할 수 있습니다.
