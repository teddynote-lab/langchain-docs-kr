...
`ElCarroLoader`와 `ElCarroDocumentSaver`로 [langchain 문서를 저장, 로드 및 삭제](/oss/integrations/document_loaders)하는 방법을 다룹니다. 이 통합은 실행 위치에 관계없이 모든 Oracle 데이터베이스에서 작동합니다.
