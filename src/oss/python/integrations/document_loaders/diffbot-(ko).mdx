...
Diffbot의 Extract API는 API token이 필요합니다. [무료 API token 받기](/oss/integrations/providers/diffbot#installation-and-setup) 지침을 따라 환경 변수를 설정하세요.
...
Knowledge Graph에 데이터를 계속 로드하려면 [`DiffbotGraphTransformer` 가이드](/oss/integrations/graphs/diffbot/#loading-the-data-into-a-knowledge-graph)를 참조하세요.