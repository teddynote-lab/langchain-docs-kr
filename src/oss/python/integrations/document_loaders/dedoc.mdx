...
표는 분할되지 않으며 각 표는 하나의 langchain @[`Document`] 객체에 해당합니다.
표의 경우 @[`Document`] 객체는 추가 `metadata` 필드 `type="table"` 및 표의 `HTML` 표현인 `text_as_html`을 가집니다.
...
첨부 파일의 경우 langchain @[`Document`] 객체는 추가 metadata 필드 `type="attachment"`를 가집니다.
