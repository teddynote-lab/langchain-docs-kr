...
이 loader는 `datadog_api_client` Python package를 사용하여 Datadog의 애플리케이션에서 로그를 가져옵니다. `Datadog API key`와 `APP key`로 loader를 초기화해야 하며, 원하는 로그를 추출하기 위한 query를 전달해야 합니다.
