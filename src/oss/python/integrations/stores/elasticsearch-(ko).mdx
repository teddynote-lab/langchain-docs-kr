...
이 문서는 Elasticsearch [key-value stores](/oss/integrations/stores)를 시작하는 데 도움이 됩니다. 모든 `ElasticsearchEmbeddingsCache` 기능 및 구성에 대한 자세한 문서는 [API reference](https://python.langchain.com/api_reference/elasticsearch/cache/langchain_elasticsearch.cache.ElasticsearchEmbeddingsCache.html)를 참조하세요.
...
다른 `ByteStores`와 마찬가지로, RAG를 위한 [문서 수집 시 영구 캐싱](/oss/integrations/text_embedding#caching)에 `ElasticsearchEmbeddingsCache` 인스턴스를 사용할 수 있습니다.
