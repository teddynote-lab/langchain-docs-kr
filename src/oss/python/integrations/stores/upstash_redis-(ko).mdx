...
이 문서는 Upstash redis [key-value stores](/oss/integrations/stores) 시작하기를 도와드립니다. 모든 `UpstashRedisByteStore` 기능 및 구성에 대한 자세한 문서는 [API reference](https://python.langchain.com/api_reference/community/storage/langchain_community.storage.upstash_redis.UpstashRedisByteStore.html)를 참조하세요.

## Overview

`UpstashRedisStore`는 [Upstash](https://upstash.com/)에서 호스팅하는 Redis 인스턴스에 모든 것을 저장하는 `ByteStore`의 구현입니다.

대신 기본 `RedisStore`를 사용하려면 [이 가이드](/oss/integrations/stores/redis/)를 참조하세요.
