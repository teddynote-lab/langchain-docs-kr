...
>[Infino](https://github.com/infinohq/infino)는 로그, 메트릭, 트레이스를 위해 설계된 확장 가능한 텔레메트리 저장소입니다. Infino는 독립형 관찰성 솔루션으로 작동하거나 관찰성 스택의 저장 계층으로 사용될 수 있습니다.

이 예제는 `LangChain`과 [Infino](https://github.com/infinohq/infino)를 통해 OpenAI 및 ChatOpenAI 모델을 호출할 때 다음을 추적하는 방법을 보여줍니다:

* prompt 입력
* `ChatGPT` 또는 다른 `LangChain` 모델의 응답
* 지연 시간
* 오류
* 소비된 토큰 수
...
## 예제 1: LangChain OpenAI Q&A; Infino에 메트릭과 로그 게시
...
이제 matplotlib을 사용하여 지연 시간, 오류 및 소비된 토큰의 그래프를 생성합니다.
...
## prompt 또는 prompt 출력에 대한 전체 텍스트 쿼리
