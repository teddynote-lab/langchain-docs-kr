...
다음은 LangChain [LLM](/oss/langchain/models) 또는 [Chat Model](/oss/langchain/models)과 함께 `TrubricsCallbackHandler`를 사용하는 두 가지 예제입니다. OpenAI 모델을 사용할 것이므로 여기에서 `OPENAI_API_KEY` 키를 설정하세요:
