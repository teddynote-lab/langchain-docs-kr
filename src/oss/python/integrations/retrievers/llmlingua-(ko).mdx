...
>[LLMLingua](https://github.com/microsoft/LLMLingua)는 컴팩트하고 잘 훈련된 language model(예: GPT2-small, LLaMA-7B)을 활용하여 prompt에서 필수적이지 않은 token을 식별하고 제거합니다. 이 접근 방식은 large language model(LLM)을 사용한 효율적인 추론을 가능하게 하며, 최소한의 성능 손실로 최대 20배의 압축을 달성합니다.

이 notebook은 LLMLingua를 document compressor로 사용하는 방법을 보여줍니다.
...
## base vector store retriever 설정

간단한 vector store retriever를 초기화하고 2023 State of the Union 연설문을 (chunk 단위로) 저장하는 것부터 시작하겠습니다. retriever가 많은 수(20개)의 문서를 검색하도록 설정할 수 있습니다.
...
이제 base retriever를 `ContextualCompressionRetriever`로 래핑하고, `LLMLinguaCompressor`를 compressor로 사용하겠습니다.
...
## LLMLingua를 사용한 QA 생성

이제 생성 단계에서 이것을 사용하는 것이 어떤 모습인지 확인할 수 있습니다
