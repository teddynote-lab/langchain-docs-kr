...
이 가이드는 Elasticsearch [retriever](/oss/langchain/retrieval)를 시작하는 데 도움을 드립니다. 모든 `ElasticsearchRetriever` 기능 및 구성에 대한 자세한 문서는 [API reference](https://python.langchain.com/api_reference/elasticsearch/retrievers/langchain_elasticsearch.retrievers.ElasticsearchRetriever.html)를 참조하세요.
...
이 retriever는 `langchain-elasticsearch` 패키지에 있습니다. 데모 목적으로 텍스트 [embeddings](/oss/integrations/text_embedding)를 생성하기 위해 `langchain-community`도 설치합니다.
...
벡터 검색의 경우, 설명을 위해 임의의 embeddings를 사용합니다. 실제 사용 사례의 경우, 사용 가능한 LangChain [Embeddings](/oss/integrations/text_embedding) 클래스 중 하나를 선택하세요.
