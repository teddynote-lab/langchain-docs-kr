...
이 페이지는 VertexAI [chat models](/oss/langchain/models) 시작하기에 대한 간단한 개요를 제공합니다. 모든 ChatVertexAI 기능 및 구성에 대한 자세한 문서는 [API reference](https://python.langchain.com/api_reference/google_vertexai/chat_models/langchain_google_vertexai.chat_models.ChatVertexAI.html)를 참조하세요.
...
Google Cloud VertexAI 통합은 [Google PaLM integration](/oss/integrations/chat/google_generative_ai/)과 별개입니다. Google은 GCP를 통해 PaLM의 엔터프라이즈 버전을 제공하기로 선택했으며, 이를 통해 제공되는 모델을 지원합니다.
