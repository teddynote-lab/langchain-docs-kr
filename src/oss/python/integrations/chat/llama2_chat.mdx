...
이 노트북은 Llama-2 `LLM`을 `Llama2Chat` wrapper로 보강하여 [Llama-2 chat prompt format](https://huggingface.co/blog/llama2#how-to-prompt-llama-2)을 지원하는 방법을 보여줍니다. LangChain의 여러 `LLM` 구현을 Llama-2 chat model의 인터페이스로 사용할 수 있습니다. 여기에는 [ChatHuggingFace](/oss/integrations/chat/huggingface), [LlamaCpp](/oss/integrations/chat/llamacpp/), [GPT4All](/oss/integrations/llms/gpt4all) 등이 포함됩니다.

`Llama2Chat`은 `BaseChatModel`을 구현하는 범용 wrapper이므로 애플리케이션에서 [chat model](/oss/langchain/models)로 사용할 수 있습니다. `Llama2Chat`은 Message 목록을 [필수 chat prompt format](https://huggingface.co/blog/llama2#how-to-prompt-llama-2)으로 변환하고 포맷된 prompt를 `str`로 래핑된 `LLM`에 전달합니다.
...
[LlamaCPP](/oss/integrations/providers/llamacpp) `LMM`과 함께 Llama-2 chat model을 사용하려면 [이 설치 지침](/oss/integrations/providers/llamacpp#installation-and-setup)을 사용하여 `llama-cpp-python` 라이브러리를 설치하세요. 다음 예제는 `~/Models/llama-2-7b-chat.Q4_0.gguf`에 로컬로 저장된 양자화된 [llama-2-7b-chat.Q4_0.gguf](https://huggingface.co/TheBloke/Llama-2-7b-Chat-GGUF/resolve/main/llama-2-7b-chat.Q4_0.gguf) model을 사용합니다.
