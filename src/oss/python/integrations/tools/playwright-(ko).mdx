...
>[Playwright](https://github.com/microsoft/playwright)는 `Microsoft`에서 개발한 오픈소스 자동화 도구로, 웹 브라우저를 프로그래밍 방식으로 제어하고 자동화할 수 있게 해줍니다. `Chromium`, `Firefox`, `WebKit`과 같은 다양한 웹 브라우저에서 엔드투엔드 테스트, 스크래핑, 작업 자동화를 위해 설계되었습니다.

이 toolkit은 브라우저와 상호작용하는 데 사용됩니다. 다른 도구들(예: `Requests` 도구)은 정적 사이트에는 적합하지만, `PlayWright Browser` toolkit을 사용하면 에이전트가 웹을 탐색하고 동적으로 렌더링되는 사이트와 상호작용할 수 있습니다.

`PlayWright Browser` toolkit에 포함된 일부 도구는 다음과 같습니다:

- `NavigateTool` (navigate_browser) - URL로 이동
- `NavigateBackTool` (previous_page) - 요소가 나타날 때까지 대기
- `ClickTool` (click_element) - 요소 클릭 (selector로 지정)
- `ExtractTextTool` (extract_text) - beautiful soup을 사용하여 현재 웹 페이지에서 텍스트 추출
- `ExtractHyperlinksTool` (extract_hyperlinks) - beautiful soup을 사용하여 현재 웹 페이지에서 하이퍼링크 추출
- `GetElementsTool` (get_elements) - CSS selector로 요소 선택
- `CurrentPageTool` (current_page) - 현재 페이지 URL 가져오기
...
context를 생성하고 브라우저를 실행하는 비동기 함수:
...
## Browser Toolkit 인스턴스화

브라우저 컨텍스트가 적절하게 초기화되고 관리되어 원활한 상호작용과 리소스 최적화를 보장하기 위해 항상 from_browser 메서드를 사용하여 인스턴스화하는 것이 권장됩니다.
...
여러 브라우저 도구들은 `StructuredTool`로, 여러 인수를 필요로 합니다. 이들은 `STRUCTURED_CHAT_ZERO_SHOT_REACT_DESCRIPTION`보다 오래된 에이전트와는 (기본적으로) 호환되지 않습니다.
