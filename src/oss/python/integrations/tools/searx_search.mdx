...
이 노트북은 자체 호스팅된 `SearxNG` search API를 사용하여 웹을 검색하는 방법을 다룹니다.

`Searx API` 매개변수에 대한 자세한 정보는 [이 링크를 확인](https://docs.searxng.org/dev/search_api.html)할 수 있습니다.
...
일부 엔진의 경우, 직접적인 `answer`를 사용할 수 있으면 wrapper는 전체 검색 결과 목록 대신 답변을 출력합니다. 모든 결과를 얻으려면 wrapper의 `results` 메서드를 사용할 수 있습니다.
...
SearxNG는 [135개의 검색 엔진](https://docs.searxng.org/user/configured_engines.html)을 지원합니다. Searx search API에 전달될 임의의 명명된 매개변수로 Searx wrapper를 사용자 정의할 수도 있습니다. 아래 예제에서는 searx search api의 custom search 매개변수를 더 흥미롭게 사용할 것입니다.

이 예제에서는 wikipedia를 쿼리하기 위해 `engines` 매개변수를 사용할 것입니다
...
`language`와 같은 searx의 다른 Searx 매개변수 전달하기
...
이 예제에서는 `categories` 매개변수를 사용하여 과학 논문을 검색하고 결과를 `time_range`로 제한할 것입니다(모든 엔진이 time range 옵션을 지원하는 것은 아닙니다).

또한 메타데이터를 포함한 구조화된 방식으로 결과를 얻고자 합니다. 이를 위해 wrapper의 `results` 메서드를 사용할 것입니다.
...
이 예제에서는 `it` 카테고리에서 `large language models`를 쿼리합니다. 그런 다음 github에서 나온 결과를 필터링합니다.
...
`github` 및 기타 source forge의 결과를 직접 쿼리할 수도 있습니다.
