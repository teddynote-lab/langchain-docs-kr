...
`Natural Language API` Toolkits (`NLAToolkits`)는 LangChain Agent가 endpoint 간 호출을 효율적으로 계획하고 결합할 수 있도록 합니다.

이 notebook은 `Speak`, `Klarna`, `Spoonacluar` API의 샘플 구성을 보여줍니다.

### 먼저, dependency를 import하고 LLM을 로드합니다
...
### 다음으로, Natural Language API Toolkits를 로드합니다
...
### Auth를 사용하고 더 많은 Endpoint를 추가합니다

일부 endpoint는 access token과 같은 것을 통한 사용자 인증이 필요할 수 있습니다. 여기서는 `Requests` wrapper 객체를 통해 인증 정보를 전달하는 방법을 보여줍니다.

각 NLATool은 래핑된 API에 대한 간결한 자연어 인터페이스를 제공하므로, 최상위 대화형 agent는 사용자의 요청을 충족하기 위해 각 endpoint를 더 쉽게 통합할 수 있습니다.

**Spoonacular endpoint를 추가합니다.**

1. [Spoonacular API Console](https://spoonacular.com/food-api/console#Profile)로 이동하여 무료 계정을 만드세요.
2. `Profile`을 클릭하고 아래에 API key를 복사하세요.
