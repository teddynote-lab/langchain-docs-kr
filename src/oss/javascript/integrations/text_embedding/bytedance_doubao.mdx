...
이 가이드는 LangChain을 사용하여 ByteDanceDoubao [embedding models](/oss/integrations/text_embedding)를 시작하는 데 도움을 드립니다. `ByteDanceDoubaoEmbeddings`의 기능 및 구성 옵션에 대한 자세한 문서는 [API reference](https://api.js.langchain.com/classes/_langchain_community.embeddings_bytedance_doubao.ByteDanceDoubaoEmbeddings.html)를 참조하세요.
...
Embedding models는 데이터 인덱싱과 나중에 검색하는 과정 모두에서 retrieval-augmented generation (RAG) 플로우에 자주 사용됩니다. 더 자세한 지침은 [**Learn** 탭](/oss/learn/)의 RAG 튜토리얼을 참조하세요.
