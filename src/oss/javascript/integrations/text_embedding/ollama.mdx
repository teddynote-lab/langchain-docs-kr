...
이 문서는 LangChain을 사용하여 Ollama [embedding models](/oss/integrations/text_embedding)를 시작하는 데 도움을 드립니다. `OllamaEmbeddings`의 기능 및 구성 옵션에 대한 자세한 문서는 [API reference](https://api.js.langchain.com/classes/langchain_ollama.OllamaEmbeddings.html)를 참조하세요.
...
Ollama embedding models에 액세스하려면 [이 지침](https://github.com/jmorganca/ollama)을 따라 Ollama를 설치하고 `@langchain/ollama` integration package를 설치해야 합니다.
...
LangChain OllamaEmbeddings integration은 `@langchain/ollama` package에 있습니다:
...
Ollama [model parameters](https://github.com/jmorganca/ollama/blob/main/docs/modelfile.md#valid-parameters-and-values)도 지원됩니다:
...
모든 `OllamaEmbeddings` 기능 및 구성에 대한 자세한 문서는 [API reference](https://api.js.langchain.com/classes/langchain_ollama.OllamaEmbeddings.html)를 참조하세요