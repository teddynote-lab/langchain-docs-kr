...
HTML 문서를 나중에 검색하기 위해 수집할 때, 우리는 종종 의미론적 요소보다는 웹페이지의 실제 콘텐츠에만 관심이 있습니다.
MozillaReadabilityTransformer로 문서에서 HTML 태그를 제거하면 더 풍부한 콘텐츠의 청크를 생성할 수 있어 검색을 더 효과적으로 만들 수 있습니다.

## Setup

[`@mozilla/readability`](https://www.npmjs.com/package/@mozilla/readability)와 [`jsdom`](https://www.npmjs.com/package/jsdom) npm package를 설치해야 합니다:

```bash npm
npm install @mozilla/readability jsdom
```
transformer 자체에는 필요하지 않지만, 아래 사용 예제에서는 스크래핑을 위해 [`cheerio`](https://www.npmjs.com/package/cheerio)가 필요합니다:
...
아래 예제는 Hacker News 스레드를 스크래핑하고, HTML 태그를 기반으로 분할하여 태그의 의미론적 정보를 기반으로 청크를 그룹화한 다음,
개별 청크에서 콘텐츠를 추출합니다:
...
transformer에 [`@mozilla/readability` package가 허용하는 인수](https://www.npmjs.com/package/@mozilla/readability)를 전달하여 작동 방식을 사용자 정의할 수 있습니다.