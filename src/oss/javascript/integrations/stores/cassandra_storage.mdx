...
이 예제는 `CassandraKVStore` @[`BaseStore`] 통합을 사용하여 채팅 기록 저장소를 설정하는 방법을 보여줍니다. 채팅 기록 저장에는 `CassandraChatMessageHistory` 통합이 더 사용하기 쉬울 수 있습니다. `CassandraKVStore`는 접두사를 붙일 수 있는 키를 가진 보다 범용적인 key-value 저장소가 필요한 경우에 유용합니다.
...
- [Key-value store 개념 가이드](/oss/integrations/stores)
```