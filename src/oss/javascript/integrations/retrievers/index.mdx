...
[Retriever](/oss/langchain/retrieval)는 비구조화된 query가 주어졌을 때 문서를 반환하는 interface입니다.
...
Retriever 사용 방법에 대한 자세한 내용은 [관련 how-to 가이드](/oss/langchain/retrieval)를 참조하세요.

모든 [vector store](/oss/integrations/vectorstores)는 [retriever로 변환](/oss/langchain/retrieval)될 수 있습니다.
사용 가능한 vector store retriever에 대해서는 vector store [통합 문서](/oss/integrations/vectorstores/)를 참조하세요.
