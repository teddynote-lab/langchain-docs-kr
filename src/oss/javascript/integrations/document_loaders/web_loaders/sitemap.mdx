...
이 노트북은 [`SitemapLoader`](https://api.js.langchain.com/classes/_langchain_community.document_loaders_web_sitemap.SitemapLoader.html) 클래스를 사용하여 sitemap을 `Document`로 로드하는 방법을 설명합니다.

## Setup

먼저 `langchain` 패키지를 설치해야 합니다:

```bash npm
npm install @langchain/community @langchain/core
```

전달된 URL은 sitemap의 `.xml` 경로를 포함하거나, 기본적으로 `/sitemap.xml`이 URL에 추가됩니다.
...
또는 sitemap의 각 페이지 내용을 로드하지 않고 sitemap만 로드하려면 `parseSitemap` 메서드를 사용할 수 있습니다:
