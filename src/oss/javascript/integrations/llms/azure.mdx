...
<Warning>
**현재 Azure OpenAI 텍스트 completion 모델 사용에 관한 페이지를 보고 계십니다. 최신이자 가장 인기 있는 Azure OpenAI 모델은 [chat completion 모델](/oss/langchain/models)입니다.**


특별히 `gpt-3.5-turbo-instruct`를 사용하는 것이 아니라면, [이 페이지](/oss/integrations/chat/azure/)를 찾고 계실 것입니다.
</Warning>
