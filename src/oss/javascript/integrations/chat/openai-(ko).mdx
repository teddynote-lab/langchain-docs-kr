...
이 가이드는 ChatOpenAI [chat models](/oss/langchain/models) 시작하기를 도와드립니다. 모든 ChatOpenAI 기능과 구성에 대한 자세한 문서는 [API reference](https://api.js.langchain.com/classes/langchain_openai.ChatOpenAI.html)를 참조하세요.
...
Azure OpenAI에서 호스팅하는 경우 [전용 페이지](/oss/integrations/chat/azure)를 참조하세요.
...
response에는 응답 텍스트와 출처를 인용하는 OpenAI [annotations](https://platform.openai.com/docs/guides/tools-web-search?api-mode=responses#output-and-citations)를 모두 포함하는 구조화된 [content blocks](/oss/langchain/messages/#message-content)가 포함됩니다. output message에는 tool 호출의 정보도 포함됩니다.
