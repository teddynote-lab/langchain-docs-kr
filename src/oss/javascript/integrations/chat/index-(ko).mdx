...
[Chat model](/oss/langchain/models)은 [message](/oss/langchain/messages) 시퀀스를 입력으로 사용하고 message를 출력으로 반환하는 language model입니다 <Tooltip tip="이름에 'Chat' 접두사가 포함되지 않거나 'LLM'이 접미사로 포함된 모델은 일반적으로 chat model 인터페이스를 따르지 않고 대신 문자열을 입력으로 받아 문자열을 출력으로 반환하는 인터페이스를 사용하는 이전 모델을 의미합니다.">(일반 텍스트와 대조적으로)</Tooltip>.
...
| :----------------------------------------------------------------------- | :----: | :-------: | :----------------------------------------: | :------------------------------------------------------------------------------------------: | :-------------------------------------------: |
| [BedrockChat](/oss/integrations/chat/bedrock/)                          |   ✅   |    ❌     |        🟡 (Bedrock Anthropic만 해당)         |                                 🟡 (Bedrock Anthropic만 해당)                                  |          🟡 (Bedrock Anthropic만 해당)          |
