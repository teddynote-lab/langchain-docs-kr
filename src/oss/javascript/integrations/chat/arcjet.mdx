...
[Arcjet](https://arcjet.com) redact integration을 사용하면 Chat Model로 전송하기 전에 프롬프트에서 민감한 사용자 정보를 삭제할 수 있습니다.

Arcjet Redact는 전적으로 사용자의 머신에서 실행되며 데이터를 다른 곳으로 전송하지 않아 최고 수준의 개인정보 보호와 성능을 보장합니다.

Arcjet Redact 객체는 그 자체로 chat model이 아니라 LLM을 래핑합니다. 입력된 텍스트를 삭제한 다음 래핑된 chat model의 출력을 반환하기 전에 삭제를 해제합니다.
...
Arcjet Redaction Library를 설치하세요:
...
이제 Arcjet Redaction으로 chat model 호출을 보호할 준비가 되었습니다!
