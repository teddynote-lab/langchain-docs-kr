...
<Tip>
Context engineering이 처음이신가요? [개념 개요](/oss/concepts/context)에서 시작하여 다양한 유형의 context와 사용 시기를 이해하세요.
</Tip>
...
LangChain [middleware](/oss/langchain/middleware)는 LangChain을 사용하는 개발자에게 context engineering을 실용적으로 만드는 기본 메커니즘입니다.
...
State를 수정하는 **persistent** 업데이트(예: [Life-cycle Context](#summarization)의 요약 예제)의 경우, `before_model` 또는 `after_model`과 같은 life-cycle hook을 사용하여 대화 기록을 영구적으로 업데이트하세요. 자세한 내용은 [middleware 문서](/oss/langchain/middleware)를 참조하세요.
</Note>
...
더 많은 예제는 [동적으로 tool 선택하기](/oss/langchain/middleware#dynamically-selecting-tools)를 참조하세요.
...
더 많은 예제는 [동적 model](/oss/langchain/agents#dynamic-model)을 참조하세요.
...
Tool에서 state, store 및 runtime context에 액세스하는 포괄적인 예제는 [Tools](/oss/langchain/tools)를 참조하세요.

## Life-cycle Context

핵심 agent 단계 **사이**에 발생하는 것을 제어합니다 - 요약, guardrail 및 로깅과 같은 교차 관심사를 구현하기 위해 데이터 흐름을 가로챕니다.

[Model Context](#model-context) 및 [Tool Context](#tool-context)에서 보았듯이, [middleware](/oss/langchain/middleware)는 context engineering을 실용적으로 만드는 메커니즘입니다. Middleware를 사용하면 agent 생명주기의 모든 단계에 연결하여 다음 중 하나를 수행할 수 있습니다:
...
<Note>
내장 middleware의 전체 목록, 사용 가능한 hook 및 사용자 정의 middleware를 만드는 방법은 [Middleware 문서](/oss/langchain/middleware)를 참조하세요.
</Note>
...
4. **내장 middleware 사용** - [`SummarizationMiddleware`](/oss/langchain/middleware#summarization), [`LLMToolSelectorMiddleware`](/oss/langchain/middleware#llm-tool-selector) 등을 활용하세요
...
- [Context 개념 개요](/oss/concepts/context) - Context 유형과 사용 시기 이해
- [Middleware](/oss/langchain/middleware) - 완전한 middleware 가이드
- [Tools](/oss/langchain/tools) - Tool 생성 및 context 액세스
- [Memory](/oss/concepts/memory) - 단기 및 장기 메모리 패턴
- [Agents](/oss/langchain/agents) - 핵심 agent 개념