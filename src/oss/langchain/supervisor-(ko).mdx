...
**supervisor 패턴**은 중앙 supervisor agent가 전문화된 worker agent들을 조정하는 [multi-agent](/oss/langchain/multi-agent) 아키텍처입니다. 이 접근 방식은 작업에 다양한 유형의 전문 지식이 필요할 때 탁월합니다. 도메인 전반에 걸쳐 tool 선택을 관리하는 하나의 agent를 구축하는 대신, supervisor가 전체 워크플로우를 이해하고 조정하는 집중된 전문가들을 만듭니다.
...
또한 사용자가 원하는 대로 작업(예: 발신 이메일)을 승인, 편집 및 거부할 수 있도록 [human-in-the-loop review](/oss/langchain/human-in-the-loop)를 통합합니다.

### supervisor를 사용하는 이유는?

Multi-agent 아키텍처를 사용하면 [tools](/oss/langchain/tools)를 worker들에게 분할할 수 있으며, 각각 고유한 prompt나 지침을 가집니다. 모든 calendar 및 email API에 직접 액세스할 수 있는 agent를 고려해보세요: 많은 유사한 tool 중에서 선택하고, 각 API의 정확한 형식을 이해하며, 여러 도메인을 동시에 처리해야 합니다. 성능이 저하되면 관련 tool과 관련 prompt를 논리적 그룹으로 분리하는 것이 도움이 될 수 있습니다(부분적으로 반복적인 개선을 관리하기 위해).
...
자세한 내용은 [Installation guide](/oss/langchain/install)를 참조하세요.
...
민감한 작업에 대한 [human-in-the-loop review](/oss/langchain/human-in-the-loop)를 통합하는 것이 신중할 수 있습니다. LangChain에는 tool 호출을 검토하기 위한 [built-in middleware](/oss/langchain/human-in-the-loop#configuring-interrupts)가 포함되어 있으며, 이 경우 sub-agent가 호출하는 tool입니다.

두 sub-agent 모두에 human-in-the-loop review를 추가해 보겠습니다:
- `create_calendar_event` 및 `send_email` tool을 중단하도록 구성하여 모든 [response types](/oss/langchain/human-in-the-loop)(`approve`, `edit`, `reject`)를 허용합니다
- **최상위 agent에만** [checkpointer](/oss/langchain/short-term-memory)를 추가합니다. 이는 실행을 일시 중지하고 재개하는 데 필요합니다.
...
@[`Command`]를 사용하여 ID를 참조하여 각 interrupt에 대한 결정을 지정할 수 있습니다. 자세한 내용은 [human-in-the-loop guide](/oss/langchain/human-in-the-loop)를 참조하세요. 시연 목적으로 여기서는 calendar 이벤트를 수락하지만 발신 이메일의 제목을 편집합니다:
...
몇 개의 tool만 있는 간단한 경우에는 단일 agent를 사용하세요. agent가 사용자와 대화해야 하는 경우 대신 [handoffs](/oss/langchain/multi-agent#handoffs)를 사용하세요. agent 간의 peer-to-peer 협업의 경우 다른 multi-agent 패턴을 고려하세요.
</Tip>

## Next steps

agent 간 대화를 위한 [handoffs](/oss/langchain/multi-agent#handoffs)에 대해 알아보고, 정보 흐름을 미세 조정하기 위한 [context engineering](/oss/langchain/context-engineering)을 탐색하고, 다양한 패턴을 비교하기 위해 [multi-agent overview](/oss/langchain/multi-agent)를 읽고, multi-agent 시스템을 디버그하고 모니터링하기 위해 [LangSmith](https://smith.langchain.com)를 사용하세요.