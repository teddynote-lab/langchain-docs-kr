...
LangGraph는 agent와 workflow를 구축할 때 [persistence](/oss/langgraph/persistence), [streaming](/oss/langgraph/streaming), 디버깅 지원 및 [deployment](/oss/langgraph/deploy)를 포함한 여러 이점을 제공합니다.

## Setup

workflow 또는 agent를 구축하려면 structured output과 tool calling을 지원하는 [모든 chat model](/oss/integrations/chat)을 사용할 수 있습니다. 다음 예제는 Anthropic을 사용합니다:
...
Workflow와 agentic 시스템은 LLM과 이에 추가하는 다양한 augmentation을 기반으로 합니다. [Tool calling](/oss/langchain/tools), [structured output](/oss/langchain/structured-output), [short term memory](/oss/langchain/short-term-memory)는 LLM을 필요에 맞게 조정하기 위한 몇 가지 옵션입니다.
...
Evaluator-optimizer workflow에서는 하나의 LLM 호출이 응답을 생성하고 다른 호출이 해당 응답을 평가합니다. evaluator 또는 [human-in-the-loop](/oss/langgraph/interrupts)가 응답에 개선이 필요하다고 판단하면 피드백이 제공되고 응답이 재생성됩니다. 이 루프는 허용 가능한 응답이 생성될 때까지 계속됩니다.
...
Agent는 일반적으로 [tool](/oss/langchain/tools)을 사용하여 작업을 수행하는 LLM으로 구현됩니다. 연속적인 피드백 루프에서 작동하며, 문제와 솔루션을 예측할 수 없는 상황에서 사용됩니다. Agent는 workflow보다 더 많은 자율성을 가지며, 사용하는 도구와 문제 해결 방법에 대한 결정을 내릴 수 있습니다. 사용 가능한 도구 세트와 agent의 동작 방식에 대한 가이드라인을 여전히 정의할 수 있습니다.

![agent.png](/oss/images/agent.png)

<Note>
Agent를 시작하려면 [quickstart](/oss/langchain/quickstart)를 참조하거나 LangChain에서 [작동 방식](/oss/langchain/agents)에 대해 자세히 읽어보세요.
</Note>
