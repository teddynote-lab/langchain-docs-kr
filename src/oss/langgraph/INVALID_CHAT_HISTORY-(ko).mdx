...
:::python
이 오류는 prebuilt @[create_agent][create_agent]에서 `call_model` graph node가 잘못된 형식의 메시지 목록을 받을 때 발생합니다. 구체적으로, `tool_calls`를 가진 `AIMessages` (LLM이 도구 호출을 요청함)가 있지만 해당하는 @[`ToolMessage`] (LLM에 반환할 도구 호출 결과)가 없을 때 잘못된 형식입니다.
:::
...
2. `tools` node로부터 업데이트(@[`ToolMessage`] 목록)를 받기 전에 graph가 중단되었고,
  ...
  * 도구 중 하나가 @[`ToolNode`] (`"tools"`)에서 처리되지 않은 오류를 발생시킨 경우
:::
...
:::python
* 기존 tool call과 일치하는 @[`ToolMessage`] 객체를 제공하고 `graph.invoke({'messages': [ToolMessage(...)]})`를 호출합니다.
...
응답되지 않은 tool call과 일치하는 `tool_call_ids`를 가진 @[`ToolMessage`] 객체를 추가합니다 3. 수정된 메시지 목록으로 `graph.update_state(config, {'messages': ...})`를 호출합니다 4. graph를 재개합니다, 예: `graph.invoke(None, config)`를 호출합니다
:::
