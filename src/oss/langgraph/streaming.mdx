...
:::python
다음 stream 모드 중 하나 이상을 리스트로 @[`stream`][CompiledStateGraph.stream] 또는 @[`astream`][CompiledStateGraph.astream] 메서드에 전달하세요:
:::

:::js
다음 stream 모드 중 하나 이상을 리스트로 @[`stream`][CompiledStateGraph.stream] 메서드에 전달하세요:
:::
...
:::python
LangGraph graph는 @[`stream`][Pregel.stream] (동기) 및 @[`astream`][Pregel.astream] (비동기) 메서드를 노출하여 stream된 출력을 iterator로 생성합니다.
...
:::js
LangGraph graph는 @[`stream`][Pregel.stream] 메서드를 노출하여 stream된 출력을 iterator로 생성합니다.
...
:::python
stream된 출력에 [subgraph](/oss/langgraph/use-subgraphs)의 출력을 포함하려면, 부모 graph의 `.stream()` 메서드에서 `subgraphs=True`를 설정할 수 있습니다. 이렇게 하면 부모 graph와 모든 subgraph의 출력이 stream됩니다.
...
Python < 3.11에서 async 코드를 사용할 때는 적절한 streaming을 활성화하기 위해 `ainvoke()`에 @[`RunnableConfig`]를 명시적으로 전달해야 합니다. 자세한 내용은 [Python < 3.11에서 Async](#async)를 참조하거나 Python 3.11+로 업그레이드하세요.
</Warning>
...
1. @[`get_stream_writer`]를 사용하여 stream writer에 액세스하고 사용자 정의 데이터를 emit합니다.
...
<Warning>
**Python < 3.11에서 async에서는 @[`get_stream_writer`] 사용 불가**
Python < 3.11에서 실행되는 async 코드에서는 @[`get_stream_writer`]가 작동하지 않습니다.
...
<Accordion title="확장 예제: 임의의 chat model streaming">
  ...
  tool call을 포함하는 @[`AIMessage`]로 graph를 호출해 봅시다:
...
2. async node 또는 tool에서 @[`get_stream_writer`]를 사용**할 수 없습니다** — `writer` 인수를 직접 전달해야 합니다.
