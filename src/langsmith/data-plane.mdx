...
PostgreSQL은 LangGraph Server의 모든 사용자, 실행 및 장기 메모리 데이터를 위한 영속성 계층입니다. 이는 checkpoint([여기](/oss/langgraph/persistence)에서 자세한 정보 참조), 서버 리소스(thread, run, assistant 및 cron), 그리고 장기 메모리 저장소에 저장된 항목([여기](/oss/langgraph/persistence#memory-store)에서 자세한 정보 참조)을 모두 저장합니다.
