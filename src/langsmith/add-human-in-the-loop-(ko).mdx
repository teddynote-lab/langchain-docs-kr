...
에이전트나 워크플로우에서 tool 호출을 검토, 편집 및 승인하려면 LangGraph의 [human-in-the-loop](/oss/langgraph/interrupts) 기능을 사용하세요.
...
```
  ...
  2. JSON 직렬화 가능한 모든 값을 @[`interrupt`] function에 전달할 수 있습니다. 여기서는 수정할 텍스트가 포함된 dict를 전달합니다.
...
* [Human-in-the-loop 개념 가이드](/oss/langgraph/interrupts): LangGraph human-in-the-loop 기능에 대해 자세히 알아보세요.
* [일반적인 패턴](/oss/langgraph/interrupts#common-patterns): 작업 승인/거부, 사용자 입력 요청, tool 호출 검토, 사람 입력 검증과 같은 패턴을 구현하는 방법을 알아보세요.