...
기본 [MessagesState](/oss/langgraph/graph-api#messagesstate)는 `AnyMessage`를 사용하며, 이는 많은 메시지 유형을 지원하지만 LLM에 직접 노출하기에는 너무 일반적입니다.
...
자세한 내용은 [low-level concepts 가이드](/oss/langgraph/graph-api#state)를 참조하세요.
