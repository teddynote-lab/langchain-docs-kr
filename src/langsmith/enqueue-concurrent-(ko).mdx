...
이 가이드는 double texting을 위한 `enqueue` 옵션을 다루며, 이는 중단 요청을 queue에 추가하고 클라이언트가 수신한 순서대로 실행합니다. 아래는 `enqueue` 옵션을 사용하는 간단한 예제입니다.

## Setup

먼저, JS와 CURL 모델 출력을 출력하기 위한 간단한 헬퍼 함수를 정의하겠습니다 (Python을 사용하는 경우 이 단계를 건너뛸 수 있습니다):
...
그런 다음, 필요한 패키지를 import하고 client, assistant, thread를 인스턴스화하겠습니다.
...
이제 두 개의 run을 시작하겠습니다. 두 번째 run은 "enqueue" multitask strategy로 첫 번째 run을 중단합니다:
...
thread에 두 run의 데이터가 모두 있는지 확인합니다:
