---
title: API로 추적하기
sidebarTitle: API로 추적하기
---

LangSmith API를 직접 사용하여 LLM 애플리케이션을 추적하는 방법을 알아보세요.

LangSmith에 trace를 전송하려면 Python 또는 TypeScript SDK를 사용하는 것을 **강력히** 권장합니다. 이러한 SDK는 batching 및 backgrounding과 같은 최적화를 통해 애플리케이션의 성능이 LangSmith로 trace를 전송하는 것에 영향을 받지 않도록 설계되었습니다. 그러나 SDK를 사용할 수 없는 경우 LangSmith REST API를 사용하여 trace를 전송할 수 있습니다. 애플리케이션에서 동기적으로 trace를 전송하면 성능에 영향을 줄 수 있습니다. 이 가이드는 LangSmith REST API를 사용하여 요청을 추적하는 방법을 보여줍니다. 전체 endpoint 목록과 request/response schema는 API 문서를 참조하세요.

## 기본 추적

run을 로깅하는 가장 간단한 방법은 POST 및 PATCH `/runs` endpoint를 사용하는 것입니다. 이러한 route는 트리 구조에 대한 최소한의 컨텍스트 정보를 요구합니다.

<Note>
LangSmith REST API를 사용할 때는 request header에 `"x-api-key"`로 API key를 제공해야 합니다.

API key가 여러 workspace에 연결되어 있는 경우, header에 `"x-tenant-id"`로 사용 중인 workspace를 지정해야 합니다.

간단한 예제에서는 request body에 `dotted_order` 또는 `trace_id` 필드를 설정할 필요가 없습니다. 이러한 필드는 시스템에 의해 자동으로 생성됩니다. 이 방법이 더 간단하지만, LangSmith에서 속도가 느리고 rate limit이 낮습니다.
</Note>
...
자세한 내용은 [Run (span) 데이터 형식](/langsmith/run-data-format) 문서를 참조하세요.

## Batch Ingestion

더 빠른 run ingestion과 더 높은 rate limit을 위해 POST `/runs/multipart` [link](https://api.smith.langchain.com/redoc#tag/run/operation/multipart_ingest_runs_api_v1_runs_multipart_post) endpoint를 사용할 수 있습니다. 아래는 예제입니다. 실행하려면 `orjson` (빠른 json 처리용)과 `requests_toolbelt`가 필요합니다.
