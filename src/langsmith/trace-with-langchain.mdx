...
사용 가능한 전체 패키지 목록은 [LangChain 문서](/oss/integrations/providers/overview)를 참고하세요.
...
<Note>
`run_name` 파라미터는 호출하는 runnable(예: 체인, 함수)의 이름만 변경합니다. LLM 객체(@[`ChatOpenAI`](`gpt-4o-mini`) 등)를 호출할 때 자동 생성되는 중첩 실행의 이름은 변경되지 않습니다.
...
또는 LangChain의 @[`RunnableConfig`]를 `RunTree.fromRunnableConfig`로 동등한 RunTree 객체로 변환하거나, @[`RunnableConfig`]를 `traceable`로 감싼 함수의 첫 번째 인자로 전달할 수 있습니다.
