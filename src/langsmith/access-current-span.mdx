```mdx
---
title: 추적된 함수 내에서 현재 run (span)에 접근하기
sidebarTitle: 추적된 함수 내에서 현재 run (span)에 접근하기
---

경우에 따라 추적된 함수 내에서 현재 run (span)에 접근하고 싶을 수 있습니다. 이는 현재 run에서 UUID, tag 또는 기타 정보를 추출하는 데 유용할 수 있습니다.

Python 또는 TypeScript SDK에서 각각 `get_current_run_tree`/`getCurrentRunTree` 함수를 호출하여 현재 run에 접근할 수 있습니다.

`RunTree` 객체에서 사용 가능한 속성의 전체 목록은 [이 참조 문서](/langsmith/run-data-format)를 확인하세요.
