...
<Tip>
**사전 요구 사항**
이 가이드는 [LangSmith](/langsmith/home), [Persistence](/oss/langgraph/persistence), 그리고 [Cross-thread persistence](/oss/langgraph/persistence#memory-store) 개념에 대한 이해를 전제로 합니다.
</Tip>

LangSmith는 [checkpoints](/oss/langgraph/persistence#checkpoints) (thread 상태)와 [cross-thread memories](/oss/langgraph/persistence#memory-store) (store items)를 모두 유지합니다. `langgraph.json`에서 Time-to-Live (TTL) 정책을 구성하여 이 데이터의 수명 주기를 자동으로 관리하고 무한정 축적되는 것을 방지할 수 있습니다.
