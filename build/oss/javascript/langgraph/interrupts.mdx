...
Interrupt는 graph 실행을 특정 지점에서 일시 중지하고 계속하기 전에 외부 입력을 기다릴 수 있게 해줍니다. 이를 통해 외부 입력이 필요한 human-in-the-loop 패턴을 구현할 수 있습니다. Interrupt가 트리거되면 LangGraph는 [persistence](/oss/javascript/langgraph/persistence) 레이어를 사용하여 graph 상태를 저장하고 실행을 재개할 때까지 무기한 대기합니다.
...
[`interrupt`](https://langchain-ai.github.io/langgraphjs/reference/functions/langgraph.interrupt-2.html) 함수는 graph 실행을 일시 중지하고 호출자에게 값을 반환합니다. node 내에서 [`interrupt`](https://langchain-ai.github.io/langgraphjs/reference/functions/langgraph.interrupt-2.html)를 호출하면 LangGraph는 현재 graph 상태를 저장하고 입력과 함께 실행을 재개할 때까지 기다립니다.

[`interrupt`](https://langchain-ai.github.io/langgraphjs/reference/functions/langgraph.interrupt-2.html)를 사용하려면 다음이 필요합니다:
...
[`interrupt`](https://langchain-ai.github.io/langgraphjs/reference/functions/langgraph.interrupt-2.html)를 호출하면 다음과 같은 일이 발생합니다:

1. **Graph 실행이 중단됩니다** [`interrupt`](https://langchain-ai.github.io/langgraphjs/reference/functions/langgraph.interrupt-2.html)가 호출된 정확한 지점에서
...
- `Command(resume=...)`에 전달된 값이 [`interrupt`](https://langchain-ai.github.io/langgraphjs/reference/functions/langgraph.interrupt-2.html) 호출의 반환 값이 됩니다
- Node는 재개될 때 [`interrupt`](https://langchain-ai.github.io/langgraphjs/reference/functions/langgraph.interrupt-2.html)가 호출된 node의 시작 부분부터 다시 시작되므로 [`interrupt`](https://langchain-ai.github.io/langgraphjs/reference/functions/langgraph.interrupt-2.html) 이전의 모든 코드가 다시 실행됩니다
...
먼저 [`interrupt`](https://langchain-ai.github.io/langgraphjs/reference/functions/langgraph.interrupt-2.html)를 사용하는 tool을 정의합니다:
...
때로는 사람의 입력을 검증하고 유효하지 않으면 다시 요청해야 합니다. 루프에서 여러 [`interrupt`](https://langchain-ai.github.io/langgraphjs/reference/functions/langgraph.interrupt-2.html) 호출을 사용하여 이를 수행할 수 있습니다.
...
Node 내에서 [`interrupt`](https://langchain-ai.github.io/langgraphjs/reference/functions/langgraph.interrupt-2.html)를 호출하면 LangGraph는 runtime에 일시 중지하라는 신호를 보내는 예외를 발생시켜 실행을 중단합니다. 이 예외는 호출 스택을 통해 전파되고 runtime에 의해 포착되어 graph에 현재 상태를 저장하고 외부 입력을 기다리도록 알립니다.

실행이 재개되면(요청된 입력을 제공한 후), runtime은 전체 node를 처음부터 다시 시작합니다—[`interrupt`](https://langchain-ai.github.io/langgraphjs/reference/functions/langgraph.interrupt-2.html)가 호출된 정확한 줄에서 재개하지 않습니다. 즉, [`interrupt`](https://langchain-ai.github.io/langgraphjs/reference/functions/langgraph.interrupt-2.html) 이전에 실행된 모든 코드가 다시 실행됩니다. 따라서 interrupt가 예상대로 작동하도록 하려면 따라야 할 몇 가지 중요한 규칙이 있습니다.



### `interrupt` 호출을 try/catch로 감싸지 마세요

[`interrupt`](https://langchain-ai.github.io/langgraphjs/reference/functions/langgraph.interrupt-2.html)가 호출 지점에서 실행을 일시 중지하는 방식은 특수 예외를 발생시키는 것입니다. [`interrupt`](https://langchain-ai.github.io/langgraphjs/reference/functions/langgraph.interrupt-2.html) 호출을 try/catch 블록으로 감싸면 이 예외를 포착하게 되어 interrupt가 graph로 다시 전달되지 않습니다.

* ✅ [`interrupt`](https://langchain-ai.github.io/langgraphjs/reference/functions/langgraph.interrupt-2.html) 호출을 오류가 발생하기 쉬운 코드와 분리하세요
...
* 🔴 [`interrupt`](https://langchain-ai.github.io/langgraphjs/reference/functions/langgraph.interrupt-2.html) 호출을 일반 try/catch 블록으로 감싸지 마세요
...
* ✅ Node 실행 전반에 걸쳐 [`interrupt`](https://langchain-ai.github.io/langgraphjs/reference/functions/langgraph.interrupt-2.html) 호출을 일관되게 유지하세요
...
* 🔴 Node 내에서 조건부로 [`interrupt`](https://langchain-ai.github.io/langgraphjs/reference/functions/langgraph.interrupt-2.html) 호출을 건너뛰지 마세요
* 🔴 실행 전반에 걸쳐 결정적이지 않은 로직을 사용하여 [`interrupt`](https://langchain-ai.github.io/langgraphjs/reference/functions/langgraph.interrupt-2.html) 호출을 반복하지 마세요
...
* ✅ [`interrupt`](https://langchain-ai.github.io/langgraphjs/reference/functions/langgraph.interrupt-2.html)에 간단한 JSON 직렬화 가능한 타입을 전달하세요
...
* 🔴 [`interrupt`](https://langchain-ai.github.io/langgraphjs/reference/functions/langgraph.interrupt-2.html)에 함수, 클래스 인스턴스 또는 기타 복잡한 객체를 전달하지 마세요
...
Interrupt는 호출된 node를 다시 실행하여 작동하므로 [`interrupt`](https://langchain-ai.github.io/langgraphjs/reference/functions/langgraph.interrupt-2.html) 이전에 호출된 부작용은 (이상적으로) 멱등성이 있어야 합니다. 참고로, 멱등성은 동일한 작업을 여러 번 적용해도 초기 실행 이후 결과가 변경되지 않는다는 것을 의미합니다.

예를 들어, node 내부에 레코드를 업데이트하는 API 호출이 있을 수 있습니다. 해당 호출이 이루어진 후 [`interrupt`](https://langchain-ai.github.io/langgraphjs/reference/functions/langgraph.interrupt-2.html)가 호출되면 node가 재개될 때 여러 번 다시 실행되어 초기 업데이트를 덮어쓰거나 중복 레코드를 생성할 수 있습니다.

* ✅ [`interrupt`](https://langchain-ai.github.io/langgraphjs/reference/functions/langgraph.interrupt-2.html) 이전에 멱등성 작업을 사용하세요
* ✅ [`interrupt`](https://langchain-ai.github.io/langgraphjs/reference/functions/langgraph.interrupt-2.html) 호출 이후에 부작용을 배치하세요
...
* 🔴 [`interrupt`](https://langchain-ai.github.io/langgraphjs/reference/functions/langgraph.interrupt-2.html) 이전에 멱등성이 없는 작업을 수행하지 마세요
...
Node 내에서 subgraph를 호출할 때, 부모 graph는 subgraph가 호출되고 [`interrupt`](https://langchain-ai.github.io/langgraphjs/reference/functions/langgraph.interrupt-2.html)가 트리거된 **node의 시작 부분**부터 실행을 재개합니다. 마찬가지로 **subgraph**도 [`interrupt`](https://langchain-ai.github.io/langgraphjs/reference/functions/langgraph.interrupt-2.html)가 호출된 node의 시작 부분부터 재개됩니다.
...
<Note>
    정적 interrupt는 human-in-the-loop 워크플로우에 **권장되지 않습니다**. 대신 [`interrupt`](https://langchain-ai.github.io/langgraphjs/reference/functions/langgraph.interrupt-2.html) 메서드를 사용하세요.
</Note>
