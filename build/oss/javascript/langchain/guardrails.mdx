...
[middleware](/oss/javascript/langchain/middleware)를 사용하여 전략적 지점(에이전트 시작 전, 완료 후, 또는 model 및 tool 호출 전후)에서 실행을 가로채는 방식으로 guardrails를 구현할 수 있습니다.
...
PII 감지 기능에 대한 자세한 내용은 [middleware 문서](/oss/javascript/langchain/middleware#pii-detection)를 참조하세요.
...
- [Middleware 문서](/oss/javascript/langchain/middleware) - 커스텀 middleware 완전 가이드
...
- [에이전트 테스트](/oss/javascript/langchain/test) - 안전성 메커니즘 테스트 전략
