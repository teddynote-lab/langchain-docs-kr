...
전체 변경 사항 목록은 [마이그레이션 가이드](/oss/javascript/migrate/langchain-v1)를 참조하세요.
...
자세한 내용은 [Agents](/oss/javascript/langchain/agents)를 참조하세요.

### Middleware

Middleware는 `createAgent`의 핵심 기능입니다. `createAgent`를 매우 커스터마이징 가능하게 만들어 구축할 수 있는 것의 한계를 높입니다.

훌륭한 에이전트는 [context engineering](/oss/javascript/langchain/context-engineering)이 필요합니다: 적절한 시기에 model에 올바른 정보를 제공하는 것입니다. Middleware는 구성 가능한 추상화를 통해 동적 prompt, 대화 요약, 선택적 tool 액세스, 상태 관리 및 guardrail을 제어하는 데 도움이 됩니다.

#### 사전 구축된 middleware

LangChain은 일반적인 패턴을 위한 몇 가지 [사전 구축된 middleware](/oss/javascript/langchain/middleware#built-in-middleware)를 제공합니다:
...
자세한 내용은 [전체 middleware 가이드](/oss/javascript/langchain/middleware)를 참조하세요.
...
- **하위 호환성**: 표준 content는 [지연 로드](/oss/javascript/langchain/messages#standard-content-blocks)될 수 있으므로 관련된 breaking change가 없습니다

자세한 내용은 [content block](/oss/javascript/langchain/messages#message-content)에 대한 가이드를 참조하세요
...
<CardGroup cols={3}>
    ...
    <Card title="Agents 문서" icon="book" href="/oss/javascript/langchain/agents" arrow>
        전체 에이전트 문서
    </Card>
    ...
    <Card title="마이그레이션 가이드" icon="arrow-right-arrow-left" href="/oss/javascript/migrate/langchain-v1" arrow>
...
- [Versioning](/oss/javascript/versioning) - 버전 번호 이해하기
- [Release policy](/oss/javascript/release-policy) - 상세한 릴리스 정책
