...
Exa(이전 Metaphor Search)는 LLM이 사용하도록 완전히 설계된 검색 엔진입니다. 자연어 쿼리를 사용하여 인터넷에서 문서를 검색한 다음 원하는 문서에서 정리된 HTML 콘텐츠를 검색할 수 있습니다.

키워드 기반 검색(Google)과 달리 Exa의 신경망 검색 기능은 쿼리를 의미론적으로 이해하고 관련 문서를 반환할 수 있습니다. 예를 들어, `"fascinating article about cats"`를 검색하고 Google과 Exa의 검색 결과를 비교할 수 있습니다. Google은 "fascinating"이라는 키워드를 기반으로 SEO 최적화된 리스티클을 제공합니다. Exa는 그냥 작동합니다.

이 페이지에서는 LangChain과 함께 `ExaSearchResults`를 사용하는 방법을 설명합니다.
...
먼저 Exa API key를 받아서 환경 변수로 추가하세요. [여기](https://dashboard.exa.ai/login)에서 가입하면 매월 1000회의 무료 검색을 받을 수 있습니다.
...
여기서는 `ExaSearchResults` tool의 인스턴스를 생성하는 방법을 보여줍니다:
...
먼저 [tool-calling model](/oss/javascript/langchain/tools)에 바인딩한 다음 호출하여 chain에서 tool을 사용할 수 있습니다:
...
`ExaRetriever`와 `createRetrieverTool`을 사용하는 LangChain tool을 만들 수 있습니다. 이러한 tool을 사용하여 모든 주제에 대한 질문에 답할 수 있는 간단한 검색 agent를 구성할 수 있습니다.

LangGraph를 사용하여 agent를 만들겠습니다. `@langchain/langgraph`가 설치되어 있는지 확인하세요:
...
그런 다음 agent와 함께 사용할 LLM을 정의합니다
...
모든 `ExaSearchResults` 기능 및 구성에 대한 자세한 문서는 [API reference](https://api.js.langchain.com/classes/langchain_exa.ExaSearchResults.html)를 참조하세요
