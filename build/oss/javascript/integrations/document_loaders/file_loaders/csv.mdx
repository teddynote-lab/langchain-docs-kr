...
이 노트북은 `CSVLoader` [document loaders](/oss/javascript/integrations/document_loaders) 시작하기에 대한 간단한 개요를 제공합니다. 모든 `CSVLoader` 기능 및 구성에 대한 자세한 문서는 [API reference](https://api.js.langchain.com/classes/langchain_community_document_loaders_fs_csv.CSVLoader.html)를 참조하세요.
