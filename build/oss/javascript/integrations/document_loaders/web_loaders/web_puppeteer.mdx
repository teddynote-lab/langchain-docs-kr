...
이 노트북은 [PuppeteerWebBaseLoader](/oss/javascript/integrations/document_loaders/) 시작하기에 대한 간단한 개요를 제공합니다. 모든 PuppeteerWebBaseLoader 기능 및 구성에 대한 자세한 문서는 [API reference](https://api.js.langchain.com/classes/langchain_community_document_loaders_web_puppeteer.PuppeteerWebBaseLoader.html)를 참조하세요.

Puppeteer는 headless Chrome 또는 Chromium을 제어하기 위한 high-level API를 제공하는 Node.js 라이브러리입니다. Puppeteer를 사용하여 웹 페이지 상호작용을 자동화할 수 있으며, JavaScript 렌더링이 필요한 동적 웹 페이지에서 데이터를 추출할 수 있습니다.

더 가벼운 솔루션을 원하고 로드하려는 웹 페이지가 JavaScript 렌더링을 필요로 하지 않는 경우, [CheerioWebBaseLoader](/oss/javascript/integrations/document_loaders/web_loaders/web_cheerio)를 대신 사용할 수 있습니다.
...
`PuppeteerWebBaseLoader` document loader에 액세스하려면 `puppeteer` peer dependency와 함께 `@langchain/community` integration package를 설치해야 합니다.
...
LangChain PuppeteerWebBaseLoader integration은 `@langchain/community` package에 있습니다:
...
다음은 PuppeteerWebBaseLoaderOptions interface를 사용하여 PuppeteerWebBaseLoader constructor에 전달할 수 있는 매개변수에 대한 설명입니다:
...
1. `launchOptions`: puppeteer.launch() method에 전달할 추가 옵션을 지정하는 선택적 객체입니다. 여기에는 브라우저를 headless mode로 실행하기 위한 headless flag 또는 Puppeteer의 동작을 느리게 하여 따라가기 쉽게 만드는 slowMo 옵션과 같은 옵션이 포함될 수 있습니다.

2. `gotoOptions`: page.goto() method에 전달할 추가 옵션을 지정하는 선택적 객체입니다. 여기에는 최대 탐색 시간을 밀리초 단위로 지정하는 timeout 옵션 또는 탐색을 성공으로 간주할 시점을 지정하는 waitUntil 옵션과 같은 옵션이 포함될 수 있습니다.

3. `evaluate`: page.evaluate() method를 사용하여 페이지에서 JavaScript 코드를 평가하는 데 사용할 수 있는 선택적 함수입니다. 이는 페이지에서 데이터를 추출하거나 페이지 요소와 상호작용하는 데 유용할 수 있습니다. 이 함수는 평가 결과를 포함하는 문자열로 resolve되는 Promise를 반환해야 합니다.

이러한 옵션을 `PuppeteerWebBaseLoader` constructor에 전달하면 loader의 동작을 사용자 정의하고 Puppeteer의 강력한 기능을 사용하여 웹 페이지를 스크랩하고 상호작용할 수 있습니다.

## Screenshots

사이트의 스크린샷을 찍으려면 위와 동일하게 loader를 초기화하고 `.screenshot()` method를 호출하세요.
이렇게 하면 페이지 콘텐츠가 base64로 인코딩된 이미지이고 metadata에 페이지 URL이 포함된 `source` 필드가 있는 `Document` 인스턴스가 반환됩니다.
...
모든 PuppeteerWebBaseLoader 기능 및 구성에 대한 자세한 문서는 [API reference](https://api.js.langchain.com/classes/langchain_community_document_loaders_web_puppeteer.PuppeteerWebBaseLoader.html)를 참조하세요.
