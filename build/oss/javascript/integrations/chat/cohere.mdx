...
[Cohere](https://cohere.com/)는 기업이 인간-기계 상호작용을 개선할 수 있도록 자연어 처리 모델을 제공하는 캐나다 스타트업입니다.

이 문서는 Cohere [chat models](/oss/javascript/langchain/models) 시작하기를 도와드립니다. 모든 `ChatCohere` 기능과 구성에 대한 자세한 문서는 [API reference](https://api.js.langchain.com/classes/langchain_cohere.ChatCohere.html)를 참조하세요.
...
LangChain.js Cohere integration을 사용하려면 API key가 필요합니다.
Cohere 계정에 가입하고 API key를 생성하려면 [여기](https://dashboard.cohere.com/welcome/register)를 방문하세요.

먼저 [`@langchain/cohere`](https://www.npmjs.com/package/@langchain/cohere) package를 설치해야 합니다.

### Credentials

[Cohere 웹사이트](https://dashboard.cohere.com/welcome/register)로 이동하여 Cohere에 가입하고 API key를 생성하세요. 완료되면 `COHERE_API_KEY` environment variable을 설정하세요:
...
LangChain ChatCohere integration은 `@langchain/cohere` package에 있습니다:
...
**참고:** 커스텀 client가 제공되면 `COHERE_API_KEY` environment variable과 constructor의 `apiKey` parameter는 모두 무시됩니다.
...
Cohere는 기본적으로 RAG 지원을 제공합니다.
문서를 context로 API 요청에 전달할 수 있으며, Cohere의 모델은 응답 생성 시 이를 사용합니다.
...
API는 정적 문서가 아닌 다른 연결도 허용합니다.
예를 들어 `web-search` connector는 쿼리를 전달하면 API가 관련 문서를 웹에서 검색합니다.
아래 예제는 이 기능을 사용하는 방법을 보여줍니다.
...
`additional_kwargs` object에서 API 요청이 몇 가지 작업을 수행한 것을 볼 수 있습니다:

- 검색 쿼리를 수행하고 결과 데이터를 `searchQueries`와 `searchResults` 필드에 저장했습니다. `searchQueries` 필드에서 더 나은 결과를 위해 쿼리를 재구성한 것을 볼 수 있습니다.
- 검색 쿼리에서 세 개의 문서를 생성했습니다.
- citation 목록을 생성했습니다.
- 위의 작업과 콘텐츠를 기반으로 최종 응답을 생성했습니다.

## API reference

모든 ChatCohere 기능과 구성에 대한 자세한 문서는 [API reference](https://api.js.langchain.com/classes/langchain_cohere.ChatCohere.html)를 참조하세요.
