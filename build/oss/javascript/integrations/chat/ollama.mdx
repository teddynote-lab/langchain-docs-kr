...
이 가이드는 `ChatOllama` [chat models](/oss/javascript/langchain/models) 시작하기를 도와드립니다. 모든 `ChatOllama` 기능 및 구성에 대한 자세한 문서는 [API reference](https://api.js.langchain.com/classes/langchain_ollama.ChatOllama.html)를 참조하세요.
...
다음과 같이 [multimodal 지원](/oss/javascript/langchain/messages#multimodal) 모델에 메시지의 `content` 필드의 일부로 이미지를 전달할 수 있습니다:
