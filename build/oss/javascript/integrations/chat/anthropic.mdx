...
이 문서는 Anthropic [chat models](/oss/javascript/langchain/models) 시작하기를 도와줍니다. 모든 `ChatAnthropic` 기능 및 구성에 대한 자세한 문서는 [API reference](https://api.js.langchain.com/classes/langchain_anthropic.ChatAnthropic.html)를 참조하세요.
...
Anthropic model과 대부분의 다른 모델 간의 주요 차이점 중 하나는 단일 Anthropic [`AIMessage`](https://v03.api.js.langchain.com/classes/_langchain_core.messages_ai_message.AIMessage.html)의 내용이 단일 문자열 또는 **content block 목록**일 수 있다는 것입니다. 예를 들어 Anthropic model이 [tool을 호출](/oss/javascript/langchain/tools)할 때, tool 호출은 메시지 내용의 일부입니다(표준화된 `AIMessage.tool_calls` 필드에도 노출됨):
...
현재 이 기능을 사용하려면 beta header로 모델을 초기화해야 합니다. 다음은 LangChain [개념 문서](/oss/javascript/concepts/)를 포함하는 system message의 일부를 캐싱하는 예제입니다:
