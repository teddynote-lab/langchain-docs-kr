...
이 가이드는 Amazon Bedrock [chat model](/oss/javascript/langchain/models) 시작하기를 도와드립니다. 모든 `BedrockChat` 기능 및 구성에 대한 자세한 문서는 [API reference](https://api.js.langchain.com/classes/langchain_community_chat_models_bedrock.BedrockChat.html)를 참조하세요.

<Tip>
더 새로운 [`ChatBedrockConverse` chat model이 전용 `@langchain/aws` integration package를 통해 사용 가능합니다](/oss/javascript/integrations/chat/bedrock_converse). 이 패키지를 사용하여 더 많은 모델에서 [tool calling](/oss/javascript/langchain/tools)을 사용할 수 있습니다.
</Tip>
...
현재 chat model integration에서는 Anthropic, Cohere, Mistral model만 지원됩니다. AI21 또는 Amazon의 foundation model에 대해서는 [text generation Bedrock variant](/oss/javascript/integrations/llms/bedrock/)를 참조하세요.
...
Bedrock model의 tool calling은 [다른 model](/oss/javascript/langchain/tools)과 유사한 방식으로 작동하지만, 모든 Bedrock model이 tool calling을 지원하는 것은 아닙니다. 자세한 내용은 [AWS model 문서](https://docs.aws.amazon.com/bedrock/latest/APIReference/welcome.html)를 참조하세요.
