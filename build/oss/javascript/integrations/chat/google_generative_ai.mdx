...
이 문서는 `ChatGoogleGenerativeAI` [chat models](/oss/javascript/langchain/models) 시작하기를 도와드립니다. 모든 `ChatGoogleGenerativeAI` 기능 및 설정에 대한 자세한 문서는 [API reference](https://api.js.langchain.com/classes/langchain_google_genai.ChatGoogleGenerativeAI.html)를 참조하세요.
...
<Tip>
LangChain VertexAI 및 VertexAI-web integration을 통해서도 Google의 `gemini` 계열 model에 액세스할 수 있습니다. 문서를 읽으려면 [여기](/oss/javascript/integrations/chat/google_vertex_ai)를 클릭하세요.
</Tip>
...
Google AI를 사용한 tool calling은 대부분 [다른 model과의 tool calling](/oss/javascript/langchain/tools)과 동일하지만 schema에 몇 가지 제한 사항이 있습니다.
