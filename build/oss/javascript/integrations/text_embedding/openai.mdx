...
이 가이드는 LangChain을 사용하여 OpenAIEmbeddings [embedding models](/oss/javascript/integrations/text_embedding)를 시작하는 데 도움을 드립니다. `OpenAIEmbeddings`의 기능 및 구성 옵션에 대한 자세한 문서는 [API reference](https://api.js.langchain.com/classes/langchain_openai.OpenAIEmbeddings.html)를 참조하세요.
...
Embedding model은 데이터 인덱싱과 나중에 검색하는 과정 모두에서 retrieval-augmented generation (RAG) flow에 자주 사용됩니다. 더 자세한 지침은 [**Learn** 탭](/oss/javascript/learn/)의 RAG tutorial을 참조하세요.
...
Azure OpenAI에서 호스팅하는 경우 [전용 페이지](/oss/javascript/integrations/text_embedding/azure_openai)를 참조하세요.
