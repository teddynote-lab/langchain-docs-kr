...
[SAP HANA Cloud Vector Engine](https://www.sap.com/events/teched/news-guide/ai.html#article8)은 `SAP HANA Cloud database`에 완전히 통합된 vector store입니다.

## Setup

먼저 [`@sap/hana-client`](https://www.npmjs.com/package/@sap/hana-client) 또는 [`hdb`](https://www.npmjs.com/package/hdb) package와 [`@langchain/community`](https://www.npmjs.com/package/@langchain/community) package를 설치해야 합니다:
...
또한 HANA Cloud instance에 대한 database 연결이 필요합니다.
...
## loader로부터 새로운 index 생성하고 유사도 검색 수행하기
...
## HNSW Vector Index 생성하기

vector index는 vector에 대한 top-k nearest neighbor query를 크게 가속화할 수 있습니다. 사용자는 `create_hnsw_index` function을 사용하여 Hierarchical Navigable Small World (HNSW) vector index를 생성할 수 있습니다.

parameter 요구사항 등 database 수준에서 index를 생성하는 것에 대한 자세한 정보는 [공식 문서](https://help.sap.com/docs/hana-cloud-database/sap-hana-cloud-sap-hana-database-vector-engine-guide/create-vector-index-statement-data-definition)를 참조하세요.
...
기본적인 값 기반 필터링 기능 외에도 더 고급 필터링을 사용할 수 있습니다. 아래 표는 사용 가능한 filter operator를 보여줍니다.
...
| `$gt`      | 큼 (&gt;)                                                        |
| `$gte`     | 크거나 같음 (&gt;=)                                              |
| `$in`      | 주어진 값의 집합에 포함됨 (in)                                    |
| `$nin`     | 주어진 값의 집합에 포함되지 않음 (not in)                            |
| `$between` | 두 경계 값의 범위 사이                                   |
| `$like`    | SQL의 "LIKE" 의미론에 기반한 텍스트 같음 ("%" 를 wildcard로 사용) |
| `$and`     | 논리 "and", 두 개 이상의 operand 지원                               |
| `$or`      | 논리 "or", 두 개 이상의 operand 지원                                |
...
## retrieval augmented generation (RAG)을 위한 chain에서 VectorStore를 retriever로 사용하기
