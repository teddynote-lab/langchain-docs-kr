...
| [`langchain.messages`](https://reference.langchain.com/python/langchain/messages)    | Message 타입, [content blocks](https://reference.langchain.com/python/langchain/messages/#langchain.messages.ContentBlock), [`trim_messages`](https://reference.langchain.com/python/langchain/messages/#langchain.messages.trim_messages) | `langchain-core`에서 재-export됨 |
| [`langchain.tools`](https://reference.langchain.com/python/langchain/tools)       | [`@tool`](https://reference.langchain.com/python/langchain/tools/#langchain.tools.tool), [`BaseTool`](https://reference.langchain.com/python/langchain/tools/#langchain.tools.BaseTool), 주입 헬퍼                   | `langchain-core`에서 재-export됨 |
| [`langchain.chat_models`](https://reference.langchain.com/python/langchain/models) | [`init_chat_model`](https://reference.langchain.com/python/langchain/models/#langchain.chat_models.init_chat_model), [`BaseChatModel`](https://reference.langchain.com/python/langchain_core/language_models/#langchain_core.language_models.chat_models.BaseChatModel)             | 통합된 모델 초기화 |
| [`langchain.embeddings`](https://reference.langchain.com/python/langchain/embeddings)  | [`init_embeddings`](https://reference.langchain.com/python/langchain_core/embeddings/#langchain_core.embeddings.embeddings.Embeddings), [`Embeddings`](https://reference.langchain.com/python/langchain_core/embeddings/#langchain_core.embeddings.embeddings.Embeddings)                | Embedding 모델 |
...
v1.0 이전에는 에이전트를 만들 때 [`langgraph.prebuilt.create_react_agent`](https://reference.langchain.com/python/langgraph/prebuilt/#langgraph.prebuilt.chat_agent_executor.create_react_agent) 사용을 권장했습니다. 이제는 에이전트를 만들 때 [`langchain.agents.create_agent`](https://reference.langchain.com/python/langchain/agents/#langchain.agents.create_agent) 사용을 권장합니다.

아래 표는 [`create_react_agent`](https://reference.langchain.com/python/langgraph/prebuilt/#langgraph.prebuilt.chat_agent_executor.create_react_agent)에서 [`create_agent`](https://reference.langchain.com/python/langchain/agents/#langchain.agents.create_agent)로 변경된 기능을 요약합니다:

| Section | TL;DR - What's changed |
|---------|--------------|
| [Import path](#import-path) | 패키지가 `langgraph.prebuilt`에서 `langchain.agents`로 이동 |
| [Prompts](#prompts) | 파라미터가 [`system_prompt`](https://reference.langchain.com/python/langchain/agents/#langchain.agents.create_agent(system_prompt))로 변경, 동적 prompts는 middleware 사용 |
...
| [Custom state](#custom-state) | `TypedDict`만 지원, [`state_schema`](https://reference.langchain.com/python/langchain/middleware/#langchain.agents.middleware.AgentMiddleware.state_schema) 또는 middleware로 정의 가능 |
...
함수 이름도 [`create_react_agent`](https://reference.langchain.com/python/langgraph/prebuilt/#langgraph.prebuilt.chat_agent_executor.create_react_agent)에서 [`create_agent`](https://reference.langchain.com/python/langchain/agents/#langchain.agents.create_agent)로 변경되었습니다:
...
`prompt` 파라미터가 [`system_prompt`](https://reference.langchain.com/python/langchain/agents/#langchain.agents.create_agent(system_prompt))로 변경되었습니다:
...
시스템 프롬프트에서 [`SystemMessage`](https://reference.langchain.com/python/langchain/messages/#langchain.messages.SystemMessage) 객체를 사용 중이라면 문자열 content를 추출하세요:
...
동적 프롬프트는 핵심 컨텍스트 엔지니어링 패턴으로, 현재 대화 state에 따라 모델에 전달하는 내용을 적응시킵니다. 이를 위해 [`@dynamic_prompt`](https://reference.langchain.com/python/langchain/middleware/#langchain.agents.middleware.dynamic_prompt) 데코레이터를 사용하세요:
...
1. **[`create_agent`](https://reference.langchain.com/python/langchain/agents/#langchain.agents.create_agent)의 [`state_schema`](https://reference.langchain.com/python/langchain/middleware/#langchain.agents.middleware.AgentMiddleware.state_schema)로** - 도구에서 사용하는 state에 최적
2. **middleware로** - 특정 middleware hooks와 해당 middleware에 연결된 도구가 관리하는 state에 최적

<Note>
middleware로 custom state를 정의하면 관련 middleware와 도구에 개념적으로 범위가 맞춰져 유지되므로, [`create_agent`](https://reference.langchain.com/python/langchain/agents/#langchain.agents.create_agent)의 [`state_schema`](https://reference.langchain.com/python/langchain/middleware/#langchain.agents.middleware.AgentMiddleware.state_schema)로 정의하는 것보다 선호됩니다.
...
커스텀 state를 도구에서 접근해야 할 때 [`state_schema`](https://reference.langchain.com/python/langchain/middleware/#langchain.agents.middleware.AgentMiddleware.state_schema) 파라미터를 사용하세요:
...
Middleware는 [`state_schema`](https://reference.langchain.com/python/langchain/middleware/#langchain.agents.middleware.AgentMiddleware.state_schema) 속성을 설정하여 custom state를 정의할 수도 있습니다.
...
middleware를 통한 custom state 정의에 대한 자세한 내용은 [middleware 문서](/oss/python/langchain/middleware#custom-state-schema)를 참조하세요.

#### State 타입 제한

[`create_agent`](https://reference.langchain.com/python/langchain/agents/#langchain.agents.create_agent)는 state 스키마로 `TypedDict`만 지원합니다. Pydantic 모델과 dataclass는 더 이상 지원되지 않습니다.
...
동적 모델 선택을 사용하면 런타임 context(예: 작업 복잡도, 비용 제약, 사용자 선호)에 따라 다른 모델을 선택할 수 있습니다. [`langgraph-prebuilt`](https://pypi.org/project/langgraph-prebuilt)의 v0.6에서 릴리스된 [`create_react_agent`](https://reference.langchain.com/python/langgraph/prebuilt/#langgraph.prebuilt.chat_agent_executor.create_react_agent)는 `model` 파라미터에 전달한 callable을 통해 동적 모델/도구 선택을 지원했습니다.
...
구조화된 출력 지원을 강화하기 위해, [`create_agent`](https://reference.langchain.com/python/langchain/agents/#langchain.agents.create_agent)는 도구나 설정이 사전 바인딩된 모델을 더 이상 허용하지 않습니다:
...
[`create_agent`](https://reference.langchain.com/python/langchain/agents/#langchain.agents.create_agent)의 [`tools`](https://reference.langchain.com/python/langchain/agents/#langchain.agents.create_agent(tools)) 인자는 다음의 리스트를 받을 수 있습니다:

* LangChain [`BaseTool`](https://reference.langchain.com/python/langchain/tools/#langchain.tools.BaseTool) 인스턴스([`@tool`](https://reference.langchain.com/python/langchain/tools/#langchain.tools.tool)로 데코레이트된 함수)
* 적절한 타입 힌트와 docstring을 갖춘 callable 객체(함수)
* 내장 provider 도구를 나타내는 `dict`

이 인자는 더 이상 [`ToolNode`](https://reference.langchain.com/python/langgraph/agents/#langgraph.prebuilt.tool_node.ToolNode) 인스턴스를 허용하지 않습니다.
...
v1에서는 메시지가 provider-agnostic 표준 content blocks를 지원합니다. provider 간 일관되고 타입이 지정된 보기를 위해 @[`message.content_blocks`][content_blocks]를 통해 접근하세요. 기존 [`message.content`](https://reference.langchain.com/python/langchain_core/language_models/#langchain_core.messages.BaseMessage.content_blocks) 필드는 문자열 또는 provider 고유의 구조에 대해 변경되지 않습니다.

### 변경 사항

- 정규화된 content에 대한 새로운 [`content_blocks`](https://reference.langchain.com/python/langchain_core/language_models/#langchain_core.messages.BaseMessage.content_blocks) 메시지 속성 추가
- [Messages](/oss/python/langchain/messages#standard-content-blocks)에 문서화된 표준화된 블록 형태
...
자세한 내용은 content blocks [레퍼런스](/oss/python/langchain/messages#content-block-reference)를 참조하세요.
...
<Note>
    더 알아보기: [Messages](/oss/python/langchain/messages#message-content), [Standard content blocks](/oss/python/langchain/messages#standard-content-blocks), [Multimodal](/oss/python/langchain/messages#multimodal).
</Note>
...
Chat 모델 호출의 반환 타입 시그니처가 [`BaseMessage`](https://reference.langchain.com/python/langchain_core/language_models/#langchain_core.messages.BaseMessage)에서 [`AIMessage`](https://reference.langchain.com/python/langchain/messages/#langchain.messages.AIMessage)로 수정되었습니다. [`bind_tools`](https://reference.langchain.com/python/langchain_core/language_models/#langchain_core.language_models.chat_models.BaseChatModel.bind_tools)를 구현하는 커스텀 chat 모델은 반환 시그니처를 업데이트해야 합니다:
...
Responses API와 상호작용할 때, `langchain-openai`는 이제 응답 항목을 메시지 `content`에 저장하는 것이 기본입니다. 이전 동작을 복원하려면 `LC_OUTPUT_VERSION` 환경 변수를 `v0`로 설정하거나, [`ChatOpenAI`](https://reference.langchain.com/python/integrations/langchain_openai/ChatOpenAI/) 인스턴스화 시 `output_version="v0"`를 지정하세요.
