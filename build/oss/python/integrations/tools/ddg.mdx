...
이 가이드는 DuckDuckGo search component를 사용하는 방법을 보여줍니다.
...
추가 정보(예: link, source)를 얻으려면 `DuckDuckGoSearchResults()`를 사용하세요
...
기본적으로 결과는 원본 검색 결과의 key-value pair를 쉼표로 구분한 문자열로 반환됩니다. `output_format="list"`를 설정하여 검색 결과를 list로 반환하거나 `output_format="json"`을 설정하여 JSON 문자열로 반환할 수도 있습니다.
...
뉴스 기사만 검색할 수도 있습니다. `backend="news"` 키워드를 사용하세요
...
검색 결과를 더 세밀하게 제어하기 위해 커스텀 `DuckDuckGoSearchAPIWrapper`를 `DuckDuckGoSearchResults`에 직접 전달할 수도 있습니다.
