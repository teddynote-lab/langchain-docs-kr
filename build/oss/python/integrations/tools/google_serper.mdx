...
이 노트북은 `Google Serper` 컴포넌트를 사용하여 웹을 검색하는 방법을 다룹니다. 먼저 [serper.dev](https://serper.dev)에서 무료 계정에 가입하고 API 키를 받아야 합니다.
...
## Self Ask With Search Agent의 일부로 사용하기

Google Serper 도구를 사용하는 agent를 생성하려면 LangGraph를 설치하세요

```python
pip install -qU langgraph langchain-openai
```

그리고 [`create_agent`](https://reference.langchain.com/python/langchain/agents/#langchain.agents.create_agent) 기능을 사용하여 ReAct agent를 초기화하세요. 또한 OpenAI의 chat model에 액세스하려면 OPENAI_API_KEY를 설정해야 합니다([platform.openai.com](https://platform.openai.com) 방문).
...
메타데이터를 포함하여 구조화된 방식으로 결과를 얻고 싶다면, wrapper의 `results` 메서드를 사용합니다.
...
이 wrapper를 사용하여 Google 이미지를 검색할 수도 있습니다. 예를 들어:
...
이 wrapper를 사용하여 Google 뉴스를 검색할 수도 있습니다. 예를 들어:
...
지난 1시간 이내에 게시된 뉴스 기사만 받고 싶다면 다음과 같이 할 수 있습니다:
...
지원되는 모든 필터를 확인하려면 [Google Search](https://google.com)로 이동하여 검색하고, "도구"를 클릭한 다음 날짜 필터를 추가하고 URL에서 "tbs="를 확인하세요.

## Google Places 검색

이 wrapper를 사용하여 Google Places를 검색할 수도 있습니다. 예를 들어:
