...
이 노트북은 Google Jobs Tool을 사용하여 현재 채용 공고를 가져오는 방법을 다룹니다.

먼저, [serpapi.com/users/sign_up](https://serpapi.com/users/sign_up)에서 `SerpApi key`에 가입하고 [serpapi.com/manage-api-key](https://serpapi.com/manage-api-key)에서 API 키를 받아야 합니다.

그런 다음 다음 명령어로 `google-search-results`를 설치해야 합니다:
    `pip install google-search-results`

그 다음 환경 변수 `SERPAPI_API_KEY`를 `SerpApi key`로 설정해야 합니다.

conda 환경을 사용하는 경우, 커널에서 다음 명령어를 사용하여 설정할 수 있습니다:
...
# ReAct agent 내에서 tool 사용하기

Google Jobs tool을 사용하는 agent를 생성하려면 LangGraph를 설치하세요
