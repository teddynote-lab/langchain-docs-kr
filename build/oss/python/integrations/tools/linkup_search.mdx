...
이 가이드는 LinkupSearchTool [tool](/oss/python/langchain/tools) 시작하기에 대한 간단한 개요를 제공합니다. LinkupSearchTool의 모든 기능과 구성에 대한 자세한 문서는 [API reference](https://python.langchain.com/api_reference/linkup/tools/linkup_langchain.search_tool.LinkupSearchTool.html)를 참조하세요.
...
Linkup provider를 사용하려면 유효한 API key가 필요하며, [여기](https://app.linkup.so/sign-up)에서 가입하여 찾을 수 있습니다. 다음 예제를 실행하려면 OpenAI API key도 필요합니다.

### Installation

이 tool은 `langchain-linkup` 패키지에 있습니다:
...
여기서는 LinkupSearchTool tool의 인스턴스를 생성하는 방법을 보여줍니다:
...
`LinkupSearchTool`의 모든 기능과 구성에 대한 자세한 문서는 [API reference](https://python.langchain.com/api_reference/linkup/tools/linkup_langchain.search_tool.LinkupSearchTool.html)를 참조하세요.
