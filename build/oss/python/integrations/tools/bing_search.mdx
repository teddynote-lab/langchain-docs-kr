...
> [Bing Search](https://learn.microsoft.com/en-us/bing/search-apis/bing-web-search/)는 Azure 서비스로, 안전하고 광고 없는 위치 인식 검색 결과를 제공하여 수십억 개의 웹 문서에서 관련 정보를 표시합니다. 단일 API 호출로 수십억 개의 웹페이지, 이미지, 비디오 및 뉴스를 검색하는 Bing의 기능을 활용하여 사용자가 월드 와이드 웹에서 원하는 정보를 찾을 수 있도록 도와줍니다.

## Setup

[지침](https://learn.microsoft.com/en-us/bing/search-apis/bing-web-search/create-bing-search-service-resource)에 따라 Azure Bing Search v7 서비스를 생성하고 subscription key를 받으세요.
...
`k` parameter를 사용하여 결과 수를 설정할 수 있습니다.
...
BingSearch를 통해 쿼리를 실행하고 snippet, title, link metadata를 반환합니다.
...
여기서는 [agent](/oss/python/langchain/agents)의 일부로 사용하는 방법을 보여줍니다. OpenAI Functions Agent를 사용하므로 필요한 종속성을 설정하고 설치해야 합니다. 또한 [LangSmith Hub](https://smith.langchain.com/hub)를 사용하여 prompt를 가져오므로 이를 설치해야 합니다.
