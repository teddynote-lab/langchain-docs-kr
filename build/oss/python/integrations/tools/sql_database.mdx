...
이 문서는 SQL Database [toolkit](/oss/python/langchain/tools#toolkits)을 시작하는 데 도움이 됩니다. 모든 `SQLDatabaseToolkit` 기능 및 구성에 대한 자세한 문서는 [API reference](https://python.langchain.com/api_reference/community/agent_toolkits/langchain_community.agent_toolkits.sql.toolkit.SQLDatabaseToolkit.html)를 참조하세요.
...
[SQL Q&A Tutorial](/oss/python/langchain/sql-agent)에 따라 아래에서는 toolkit의 도구를 간단한 질문-답변 에이전트에 장착합니다. 먼저 관련 프롬프트를 가져와서 필요한 매개변수로 채웁니다:
