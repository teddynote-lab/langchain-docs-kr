...
이 가이드는 [langchain_discord](/oss/python/integrations/tools/)에서 Discord 도구를 시작하기 위한 빠른 개요를 제공합니다. 각 도구 및 구성에 대한 자세한 내용은 저장소의 docstring 또는 관련 문서 페이지를 참조하세요.
...
다음은 LLM과 함께 chain 또는 agent에서 `DiscordReadMessages` 및 `DiscordSendMessage` 도구를 통합하는 방법을 보여주는 보다 완전한 예제입니다. 이 예제는 적절한 경우 도구를 호출할 수 있는 LangChain 스타일의 agent를 설정하는 함수(예: [`create_agent`](https://reference.langchain.com/python/langchain/agents/#langchain.agents.create_agent))가 있다고 가정합니다.
