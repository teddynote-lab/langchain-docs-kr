...
Google Books tool은 ReAct 패턴을 지원하며 Google Books API를 검색할 수 있게 해줍니다. Google Books는 세계에서 가장 큰 API로, 책을 체계적으로 관리하고 추적합니다. 4천만 개 이상의 항목을 보유하고 있어 사용자에게 상당한 양의 데이터를 제공할 수 있습니다.

### Tool 기능

현재 이 tool은 다음과 같은 기능을 제공합니다:

- 키워드 검색을 사용하여 Google Books API에서 관련 정보를 수집합니다
- 정보를 읽기 쉬운 출력 형식으로 포맷하고 결과를 agent에 반환합니다
...
Google Books에서 API key가 필요합니다. [https://developers.google.com/books/docs/v1/using#APIKey](https://developers.google.com/books/docs/v1/using#APIKey)를 방문하여 단계를 따라 진행하면 됩니다.

그런 다음 환경 변수 `GOOGLE_BOOKS_API_KEY`를 Google Books API key로 설정해야 합니다.

## Instantiation

tool을 인스턴스화하려면 Google Books tool을 import하고 credentials를 설정하세요.
...
`run` method를 호출하여 tool을 실행할 수 있습니다.
...
### [args와 함께 직접 호출하기](/oss/python/langchain/tools)
...
### [ToolCall과 함께 호출하기](/oss/python/langchain/tools)

tool call 예제는 아래를 참조하세요.
...
Google Books API는 여기에서 찾을 수 있습니다: [https://developers.google.com/books](https://developers.google.com/books)
