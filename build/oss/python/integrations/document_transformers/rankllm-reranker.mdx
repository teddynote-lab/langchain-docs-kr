...
**[RankLLM](https://github.com/castorini/rank_llm)**은 **listwise, pairwise, pointwise ranking model을 지원하는 유연한 reranking framework**입니다. **RankVicuna, RankZephyr, MonoT5, DuoT5, LiT5, FirstMistral**을 포함하며, 효율적인 inference를 위해 **FastChat, vLLM, SGLang, TensorRT-LLM**과 통합됩니다. RankLLM은 **retrieval 및 ranking 작업**에 최적화되어 있으며, **오픈소스 LLM**과 **RankGPT, RankGemini**와 같은 독점 reranker를 모두 활용합니다. **batched inference, first-token reranking, BM25 및 SPLADE를 통한 retrieval**을 지원합니다.

> **참고:** 내장 retriever를 사용하는 경우, RankLLM은 retrieval 기능을 위해 **Pyserini, JDK 21, PyTorch, Faiss**가 필요합니다.
...
RankZephyr는 향상된 retrieval 품질을 위해 listwise reranking을 수행하지만, 효율적으로 실행하려면 최소 24GB의 VRAM이 필요합니다.
...
RankGPT를 사용한 Retrieval + Reranking
...
이 retriever를 QA pipeline 내에서 사용할 수 있습니다
