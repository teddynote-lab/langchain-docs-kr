...
[Doctran](https://github.com/psychic-api/doctran) 라이브러리를 사용하여 문서의 유용한 특징을 추출할 수 있습니다. 이 라이브러리는 OpenAI의 function calling 기능을 사용하여 특정 메타데이터를 추출합니다.

문서에서 메타데이터를 추출하는 것은 다음과 같은 다양한 작업에 유용합니다:

* **분류:** 문서를 다양한 카테고리로 분류
* **데이터 마이닝:** 데이터 분석에 사용할 수 있는 구조화된 데이터 추출
* **스타일 변환:** 예상되는 사용자 입력과 더 밀접하게 일치하도록 텍스트 작성 방식을 변경하여 벡터 검색 결과 개선
