...
## OpenVINO로 Reranking하기

이제 `OpenVINOReranker`를 compressor로 사용하여 기본 retriever를 `ContextualCompressionRetriever`로 래핑해 보겠습니다.
...
reranking 후, 상위 4개 문서는 기본 retriever가 검색한 상위 4개 문서와 다릅니다.
...
`OVModelForSequenceClassification`을 사용하여 rerank 모델을 OpenVINO IR 형식으로 내보내고 로컬 폴더에서 모델을 로드할 수 있습니다.
