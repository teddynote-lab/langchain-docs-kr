...
>
>`PromptLayer`는 LangChain과 직접 통합되는 LLM을 제공하지만(예: [`PromptLayerOpenAI`](/oss/python/integrations/llms/promptlayer_openai)), callback을 사용하는 것이 `PromptLayer`를 LangChain과 통합하는 권장 방법입니다.
...
이 간단한 예제에서는 [`ChatOpenAI`](https://reference.langchain.com/python/integrations/langchain_openai/ChatOpenAI/)와 함께 `PromptLayerCallbackHandler`를 사용합니다. `chatopenai`라는 PromptLayer tag를 추가합니다.
