...
이 노트북은 통합된 [vector database](https://learn.microsoft.com/en-us/azure/cosmos-db/vector-database)를 활용하여 collection에 문서를 저장하고, index를 생성하며, COS(코사인 거리), L2(유클리드 거리), IP(내적)와 같은 근사 최근접 이웃 알고리즘을 사용하여 쿼리 벡터에 가까운 문서를 찾는 vector search 쿼리를 수행하는 방법을 보여줍니다.

Azure Cosmos DB는 OpenAI의 ChatGPT 서비스를 지원하는 데이터베이스입니다. 한 자릿수 밀리초 응답 시간, 자동 및 즉각적인 확장성, 그리고 모든 규모에서 보장된 속도를 제공합니다.

Azure Cosmos DB for NoSQL은 이제 미리 보기로 vector indexing 및 search를 제공합니다. 이 기능은 고차원 벡터를 처리하도록 설계되어 모든 규모에서 효율적이고 정확한 vector search를 가능하게 합니다. 이제 데이터와 함께 문서에 벡터를 직접 저장할 수 있습니다. 즉, 데이터베이스의 각 문서는 기존의 스키마 없는 데이터뿐만 아니라 문서의 다른 속성으로 고차원 벡터도 포함할 수 있습니다. 이러한 데이터와 벡터의 공동 배치는 벡터가 나타내는 데이터와 동일한 논리적 단위에 저장되므로 효율적인 indexing 및 searching을 가능하게 합니다. 이는 데이터 관리, AI 애플리케이션 아키텍처 및 벡터 기반 작업의 효율성을 단순화합니다.
...
## AzureCosmosDB NoSQL Vector Search 생성
...
## 사용자 정의 projection을 사용한 Hybrid Search
