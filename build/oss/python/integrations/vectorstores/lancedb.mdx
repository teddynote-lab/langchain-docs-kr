...
>[LanceDB](https://lancedb.com/)는 persistent storage를 기반으로 한 vector-search용 오픈 소스 데이터베이스로, embeddings의 retrieval, filtering 및 관리 작업을 크게 단순화합니다. 완전한 오픈 소스입니다.

이 노트북은 Lance 데이터 포맷을 기반으로 하는 `LanceDB` vector database 관련 기능을 사용하는 방법을 보여줍니다.
...
우리는 [`OpenAIEmbeddings`](https://reference.langchain.com/python/integrations/langchain_openai/OpenAIEmbeddings/)를 사용하려고 하므로 OpenAI API Key를 얻어야 합니다.
...
##### LanceDB cloud의 경우, 다음과 같이 vector store를 호출할 수 있습니다
...
또한 `from_documents()` classmethod에 `region`, `api_key`, `uri`를 추가할 수도 있습니다
...
또한, 테이블을 탐색하기 위해 df로 로드하거나 csv 파일로 저장할 수 있습니다:
