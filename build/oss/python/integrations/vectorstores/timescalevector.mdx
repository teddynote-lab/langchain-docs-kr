...
TimescaleVector store를 초기화한 후에는 이를 [retriever](/oss/python/langchain/retrieval)로 사용할 수 있습니다.
...
다음으로 JSON 레코드에서 관련 메타데이터를 추출하는 metadata function을 정의합니다. 이 함수를 JSONLoader에 전달합니다. 자세한 내용은 [JSON document loader 문서](/oss/python/integrations/document_loaders/json)를 참고하세요.
