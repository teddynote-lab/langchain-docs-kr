...
>[Lantern](https://github.com/lanterndata/lantern)는 `Postgres`용 오픈 소스 vector similarity search입니다

다음 기능을 지원합니다:

- Exact 및 approximate nearest neighbor search
- L2 squared distance, hamming distance, 그리고 cosine distance

이 통합을 사용하려면 `pip install -qU langchain-community`로 `langchain-community`를 설치해야 합니다

이 노트북은 Postgres vector database(`Lantern`) 사용 방법을 보여줍니다.

[설치 안내](https://github.com/lanterndata/lantern#-quick-install)를 참고하세요.
...
## Cosine Distance(기본값)을 사용한 Similarity Search
...
Maximal marginal relevance는 쿼리와의 유사도와 선택된 문서들 간의 다양성을 동시에 최적화합니다.
...
위에서는 처음부터 vectorstore를 생성했습니다. 그러나 실제로는 기존 vectorstore와 함께 작업해야 하는 경우가 많습니다.
이를 위해서는 기존 vectorstore를 직접 초기화할 수 있습니다.
...
기존 collection이 있는 경우 `from_documents`를 사용하고 `pre_delete_collection` = True로 설정하여 덮어쓸 수 있습니다.
이렇게 하면 다시 채우기 전에 해당 collection이 삭제됩니다.
