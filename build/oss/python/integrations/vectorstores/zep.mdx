...
- [`Embeddings`](https://reference.langchain.com/python/langchain_core/embeddings/#langchain_core.embeddings.embeddings.Embeddings) 인스턴스를 전달하면 Zep은 문서를 자동으로 embedding하는 대신 이를 사용하여 문서를 embedding합니다.
문서 collection을 `isAutoEmbedded === false`로 설정해야 합니다.
- collection을 `isAutoEmbedded === false`로 설정하면 [`Embeddings`](https://reference.langchain.com/python/langchain_core/embeddings/#langchain_core.embeddings.embeddings.Embeddings) 인스턴스를 전달해야 합니다.
