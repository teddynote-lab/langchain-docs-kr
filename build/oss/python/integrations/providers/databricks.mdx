...
3. 📊 **MLflow** - [MLflow](https://mlflow.org/)는 실험 관리, 평가, 추적, 배포 등을 포함한 전체 ML 라이프사이클을 관리하는 오픈소스 플랫폼입니다. [MLflow의 LangChain 통합](/oss/python/integrations/providers/mlflow_tracking)은 현대적인 복합 ML 시스템을 개발하고 운영하는 프로세스를 간소화합니다.
...
LangChain 애플리케이션 내에서 사용하는 방법에 대한 자세한 가이드는 [사용 예제](/oss/python/integrations/chat/databricks)를 참조하세요.
...
<Warning>
텍스트 completion 모델은 deprecated되었으며 최신 및 가장 인기 있는 모델은 [chat completion 모델](/oss/python/langchain/models)입니다. 이러한 모델과 tool calling과 같은 고급 기능을 사용하려면 `ChatDatabricks` chat model을 대신 사용하세요.
</Warning>
...
LangChain 애플리케이션 내에서 사용하는 방법에 대한 자세한 가이드는 [사용 예제](/oss/python/integrations/llms/databricks)를 참조하세요.
...
LangChain 애플리케이션 내에서 사용하는 방법에 대한 자세한 가이드는 [사용 예제](/oss/python/integrations/text_embedding/databricks)를 참조하세요.
...
벡터 인덱스를 설정하고 LangChain과 통합하는 방법은 [사용 예제](/oss/python/integrations/vectorstores/databricks_vector_search)를 참조하세요.
...
광범위한 코드 예제와 가이드를 통해 LangChain과 함께 MLflow를 사용하는 전체 기능에 대해 알아보려면 [MLflow LangChain 통합](/oss/python/integrations/providers/mlflow_tracking)을 참조하세요.
...
HuggingFace에 호스팅된 Databricks의 오픈 모델을 직접 통합하려면 LangChain의 [HuggingFace 통합](/oss/python/integrations/providers/huggingface)을 사용할 수 있습니다.
