...
>[Tavily's Search API](https://tavily.com)는 AI 에이전트(LLM)를 위해 특별히 구축된 검색 엔진으로, 실시간으로 정확하고 사실적인 결과를 빠르게 제공합니다.

이를 [retriever](/oss/python/langchain/retrieval)로 사용할 수 있습니다. 이 통합에 특화된 기능을 보여드리겠습니다. 진행한 후에는 [관련 사용 사례 페이지](/oss/python/langchain/rag)를 탐색하여 이 vectorstore를 더 큰 chain의 일부로 사용하는 방법을 배우는 것이 유용할 수 있습니다.
...
이 통합은 `langchain-community` 패키지에 포함되어 있습니다. 또한 `tavily-python` 패키지 자체도 설치해야 합니다.

```python
pip install -qU langchain-community tavily-python
```

Tavily API key도 설정해야 합니다.
...
모든 `TavilySearchAPIRetriever` 기능과 구성에 대한 자세한 문서는 [API reference](https://python.langchain.com/api_reference/community/retrievers/langchain_community.retrievers.tavily_search_api.TavilySearchAPIRetriever.html)를 참조하세요.
