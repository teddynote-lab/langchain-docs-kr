...
이 노트북은 retriever에서 [Cohere의 rerank endpoint](https://docs.cohere.com/docs/reranking)를 사용하는 방법을 보여줍니다.
...
## CohereRerank로 reranking 수행하기

이제 기본 retriever를 `ContextualCompressionRetriever`로 래핑해보겠습니다. Cohere rerank endpoint를 사용하여 반환된 결과를 재순위화하는 `CohereRerank`를 추가하겠습니다.
CohereRerank에서 model 이름을 지정하는 것이 필수라는 점에 유의하세요!
