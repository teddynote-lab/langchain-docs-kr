---
title: LangChain을 위한 Redis Cache
---

이 노트북은 langchain-redis 패키지의 `RedisCache`와 `RedisSemanticCache` 클래스를 사용하여 LLM 응답에 대한 캐싱을 구현하는 방법을 보여줍니다.

## Setup

먼저 필요한 의존성을 설치하고 Redis 인스턴스가 실행 중인지 확인하겠습니다.

```python
pip install -U langchain-core langchain-redis langchain-openai redis
```

Redis 서버가 실행 중인지 확인하세요. Docker를 사용하여 다음과 같이 시작할 수 있습니다:

```
docker run -d -p 6379:6379 redis:latest
```

또는 운영 체제의 지침에 따라 Redis를 로컬에 설치하고 실행하세요.
...
이 노트북은 langchain-redis 패키지의 `RedisCache`와 `RedisSemanticCache` 사용법을 보여주었습니다. 이러한 캐싱 메커니즘은 중복된 API 호출을 줄이고 지능적인 캐싱을 위해 의미론적 유사성을 활용함으로써 LLM 기반 애플리케이션의 성능을 크게 향상시킬 수 있습니다. Redis 기반 구현은 분산 시스템에서 캐싱을 위한 빠르고 확장 가능하며 유연한 솔루션을 제공합니다.
