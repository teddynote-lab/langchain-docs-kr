...
이 노트북은 [Bigtable](https://cloud.google.com/bigtable)을 사용하여 `BigtableLoader`와 `BigtableSaver`로 [langchain 문서를 저장, 로드 및 삭제](/oss/python/integrations/document_loaders)하는 방법을 다룹니다.
...
기본적으로 [`Document`](https://reference.langchain.com/python/langchain_core/documents/#langchain_core.documents.base.Document) 객체의 `metadata` map에는 행의 rowkey 값을 값으로 하는 `rowkey`라는 단일 키가 포함됩니다. 해당 map에 더 많은 항목을 추가하려면 metadata_mapping을 사용하세요.
