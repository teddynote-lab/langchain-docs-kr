...
[FireCrawl](https://firecrawl.dev/?ref=langchain)은 모든 웹사이트를 크롤링하고 LLM에 적합한 데이터로 변환합니다. 접근 가능한 모든 하위 페이지를 크롤링하고 각 페이지에 대한 깔끔한 markdown과 메타데이터를 제공합니다. sitemap이 필요하지 않습니다.
...
자신의 API key를 받아야 합니다. [firecrawl.dev](https://firecrawl.dev)를 참조하세요.
...
- `scrape`: 단일 url을 스크랩하고 markdown을 반환합니다.
- `crawl`: url과 접근 가능한 모든 하위 페이지를 크롤링하고 각각에 대한 markdown을 반환합니다.
- `map`: URL을 매핑하고 의미적으로 관련된 페이지 목록을 반환합니다.
...
loader에 `params`를 전달할 수도 있습니다. 이것은 crawler에 전달할 옵션의 dictionary입니다. 자세한 내용은 [FireCrawl API documentation](https://github.com/mendableai/firecrawl-py)을 참조하세요.
...
loader에 `params`를 전달할 수도 있습니다. 이것은 loader에 전달할 옵션의 dictionary입니다. 자세한 내용은 [FireCrawl API documentation](https://github.com/mendableai/firecrawl-py)을 참조하세요.

## API reference

모든 `FireCrawlLoader` 기능 및 구성에 대한 자세한 문서는 API reference를 참조하세요: [python.langchain.com/api_reference/community/document_loaders/langchain_community.document_loaders.firecrawl.FireCrawlLoader.html](https://python.langchain.com/api_reference/community/document_loaders/langchain_community.document_loaders.firecrawl.FireCrawlLoader.html)
