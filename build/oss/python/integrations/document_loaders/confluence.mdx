...
이 loader를 사용하면 Confluence 페이지를 가져와서 [`Document`](https://reference.langchain.com/python/langchain_core/documents/#langchain_core.documents.base.Document) 객체로 처리할 수 있습니다.
...
boolean 매개변수 **include_attachments**를 `True`로 설정하여 로드된 [`Document`](https://reference.langchain.com/python/langchain_core/documents/#langchain_core.documents.base.Document) 객체에 첨부 파일을 포함할 수 있습니다 (기본값: `False`). 활성화되면 모든 첨부 파일이 다운로드되고 텍스트 콘텐츠가 추출되어 Document에 추가됩니다.
