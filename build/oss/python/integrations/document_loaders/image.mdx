...
이 문서는 이미지를 다른 LangChain 모듈과 함께 사용할 수 있는 문서 형식으로 로드하는 방법을 다룹니다.

`.jpg` 및 `.png`와 같은 다양한 이미지 형식을 처리하기 위해 [Unstructured](https://unstructured.io/)를 사용합니다. Unstructured를 로컬에서 설정하는 방법(필수 시스템 종속성 설정 포함)에 대한 자세한 내용은 [이 가이드](/oss/python/integrations/providers/unstructured/)를 참조하세요.
...
내부적으로 Unstructured는 텍스트의 여러 청크에 대해 서로 다른 "element"를 생성합니다. 기본적으로 이들을 결합하지만, `mode="elements"`를 지정하여 분리된 상태로 유지할 수 있습니다.
