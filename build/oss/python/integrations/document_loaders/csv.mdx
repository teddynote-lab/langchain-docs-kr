...
>[comma-separated values (CSV)](https://en.wikipedia.org/wiki/Comma-separated_values) 파일은 쉼표를 사용하여 값을 구분하는 구분 기호로 구분된 텍스트 파일입니다. 파일의 각 줄은 데이터 레코드입니다. 각 레코드는 쉼표로 구분된 하나 이상의 필드로 구성됩니다.

[csv](https://en.wikipedia.org/wiki/Comma-separated_values) 데이터를 문서당 한 행씩 로드합니다.
...
지원되는 csv 인자에 대한 자세한 내용은 [csv module](https://docs.python.org/3/library/csv.html) 문서를 참조하세요.
...
각 행에서 생성된 문서의 소스를 지정하려면 `source_column` 인자를 사용하세요. 그렇지 않으면 CSV 파일에서 생성된 모든 문서의 소스로 `file_path`가 사용됩니다.

이는 소스를 사용하여 질문에 답변하는 chain에서 CSV 파일에서 로드된 문서를 사용할 때 유용합니다.
...
`UnstructuredCSVLoader`를 사용하여 테이블을 로드할 수도 있습니다. `UnstructuredCSVLoader`를 사용하는 한 가지 장점은 `"elements"` 모드에서 사용하면 테이블의 HTML 표현이 metadata에서 사용 가능하다는 것입니다.
