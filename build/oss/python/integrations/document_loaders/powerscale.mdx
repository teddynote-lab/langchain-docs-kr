...
- 반환된 [`Document`](https://reference.langchain.com/python/langchain_core/documents/#langchain_core.documents.base.Document)의 `metadata` 필드는 수정된 파일이 포함된 PowerScale의 경로를 반환합니다. 이 경로를 사용하여 NFS(또는 S3)를 통해 데이터를 읽고 애플리케이션에서 데이터를 처리할 수 있습니다(예: 청크 및 임베딩 생성).
...
load 함수와 동일한 [`Document`](https://reference.langchain.com/python/langchain_core/documents/#langchain_core.documents.base.Document)가 반환되며 위에서 언급한 모든 속성이 동일합니다.
