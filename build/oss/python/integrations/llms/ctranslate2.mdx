...
**CTranslate2**는 Transformer model의 효율적인 추론을 위한 C++ 및 Python library입니다.

이 프로젝트는 가중치 양자화, layer fusion, batch reordering 등과 같은 많은 성능 최적화 기법을 적용하여 CPU와 GPU에서 Transformer model의 속도를 높이고 메모리 사용량을 줄이는 custom runtime을 구현합니다.

전체 기능 목록과 지원되는 model은 [프로젝트 repository](https://opennmt.net/CTranslate2/guides/transformers.html)에 포함되어 있습니다. 시작하려면 공식 [quickstart guide](https://opennmt.net/CTranslate2/quickstart.html)를 확인하세요.

사용하려면 `ctranslate2` python package가 설치되어 있어야 합니다.

```python
pip install -qU  ctranslate2
```

Hugging Face model을 CTranslate2와 함께 사용하려면 먼저 `ct2-transformers-converter` command를 사용하여 CTranslate2 형식으로 변환해야 합니다. 이 command는 사전 학습된 model 이름과 변환된 model directory의 경로를 인자로 받습니다.
