...
<Warning>
**현재 Azure OpenAI text completion 모델 사용에 관한 페이지를 보고 계십니다. 최신이자 가장 인기 있는 Azure OpenAI 모델은 [chat completion 모델](/oss/python/langchain/models)입니다.**

`gpt-3.5-turbo-instruct`를 특별히 사용하는 경우가 아니라면, [이 페이지](/oss/python/integrations/chat/azure_chat_openai/)를 찾고 계실 것입니다.
</Warning>
...
_**참고**: 이 문서는 Azure text completion 모델에 대한 것입니다. GPT-4와 같은 모델은 chat 모델입니다. 약간 다른 인터페이스를 가지고 있으며 [`AzureChatOpenAI`](https://reference.langchain.com/python/integrations/langchain_openai/AzureChatOpenAI/) 클래스를 통해 액세스할 수 있습니다. Azure chat에 대한 문서는 [Azure Chat OpenAI 문서](/oss/python/integrations/chat/azure_chat_openai)를 참조하세요._
