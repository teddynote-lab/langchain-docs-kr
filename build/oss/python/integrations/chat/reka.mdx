...
이 가이드는 Reka [chat models](/oss/python/langchain/models) 시작하기에 대한 간단한 개요를 제공합니다.
...
model_with_tools가 아닌 model을 전달하고 있다는 점에 유의하세요. 이는 [`create_agent`](https://reference.langchain.com/python/langchain/agents/#langchain.agents.create_agent)가 내부적으로 `.bind_tools`를 호출하기 때문입니다.
