...
이 가이드는 Anthropic (Claude) [chat models](/oss/python/langchain/models) 시작하기에 대한 간단한 개요를 제공합니다.
...
<Tip>
    **API Reference**
    ...
    모든 기능과 구성 옵션에 대한 자세한 문서는 [`ChatAnthropic`](https://reference.langchain.com/python/integrations/langchain_anthropic/ChatAnthropic/) API reference를 참조하세요.
</Tip>

<Info>
    **AWS Bedrock과 Google VertexAI**

    특정 Anthropic 모델은 AWS Bedrock과 Google VertexAI를 통해서도 액세스할 수 있습니다. 이러한 서비스를 통해 Anthropic 모델을 사용하려면 [`ChatBedrock`](/oss/python/integrations/chat/bedrock/) 및 [`ChatVertexAI`](/oss/python/integrations/chat/google_vertex_ai_palm/) integration을 참조하세요.
</Info>
...
tool, [extended thinking](#extended-thinking) 및 기타 기능을 사용할 때, 단일 Anthropic [`AIMessage`](https://reference.langchain.com/python/langchain/messages/#langchain.messages.AIMessage)의 content는 단일 문자열이거나 content block 목록일 수 있습니다. 예를 들어, Anthropic 모델이 tool을 호출할 때, tool invocation은 message content의 일부입니다(표준화된 `AIMessage.tool_calls`에도 노출됨):
...
Claude는 Anthropic의 native format([vision](https://docs.claude.com/en/docs/build-with-claude/vision#base64-encoded-image-example) 및 [PDF support](https://docs.claude.com/en/docs/build-with-claude/pdf-support) 문서 참조)과 LangChain의 [표준 형식](/oss/python/langchain/messages#multimodal) 모두에서 image 및 PDF 입력을 content block으로 지원합니다.
...
extended thinking을 사용하려면 [`ChatAnthropic`](https://reference.langchain.com/python/integrations/langchain_anthropic/ChatAnthropic/)을 초기화할 때 `thinking` parameter를 지정하세요. invocation 중에 `kwarg`로 전달할 수도 있습니다.
...
아래에서는 이 기능을 통합한 간단한 chatbot을 구현합니다. LangChain [chatbot tutorial](/oss/python/langchain/quickstart)을 따르지만, 각 user message의 마지막 content block을 `cache_control`로 자동으로 표시하는 custom [reducer](/oss/python/langgraph/graph-api#reducers)를 추가합니다. 아래를 참조하세요:
...
여기서는 샘플 문서로 LangChain [vector store](/oss/python/integrations/vectorstores/)를 채우고 해당 문서를 query하는 tool을 Claude에 장착하는 end-to-end 예제를 보여줍니다.
...
Anthropic은 [custom document](https://docs.claude.com/en/docs/build-with-claude/citations#custom-content-documents) type을 사용하여 자체 split을 지정할 수도 있습니다. LangChain [text splitter](/oss/python/integrations/splitters/)를 사용하여 이 목적을 위한 의미 있는 split을 생성할 수 있습니다. 아래 예제를 참조하세요. 여기서는 LangChain README(markdown 문서)를 split하고 context로 Claude에 전달합니다:
...
Anthropic은 다양한 [built-in tool](https://docs.claude.com/en/docs/agents-and-tools/tool-use/text-editor-tool)을 지원하며, [일반적인 방법](/oss/python/langchain/tools/)으로 model에 bind할 수 있습니다. Claude는 tool에 대한 내부 schema를 준수하는 tool call을 생성합니다:
