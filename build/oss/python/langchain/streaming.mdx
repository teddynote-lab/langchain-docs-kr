...
Agent progress를 streaming하려면 `stream_mode="updates"`와 함께 [`stream`](https://reference.langchain.com/python/langgraph/graphs/#langgraph.graph.state.CompiledStateGraph.stream) 또는 [`astream`](https://reference.langchain.com/python/langgraph/graphs/#langgraph.graph.state.CompiledStateGraph.astream) 메서드를 사용하세요. 이는 모든 agent 단계 후에 이벤트를 전송합니다.
...
* **LLM node**: tool call 요청이 포함된 [`AIMessage`](https://reference.langchain.com/python/langchain/messages/#langchain.messages.AIMessage)
* **Tool node**: 실행 결과가 포함된 [`ToolMessage`](https://reference.langchain.com/python/langchain/messages/#langchain.messages.ToolMessage)
* **LLM node**: 최종 AI 응답
...
Tool이 실행될 때 업데이트를 streaming하려면 [`get_stream_writer`](https://reference.langchain.com/python/langgraph/config/#langgraph.config.get_stream_writer)를 사용할 수 있습니다.
...
<Note>
    Tool 내부에 [`get_stream_writer`](https://reference.langchain.com/python/langgraph/config/#langgraph.config.get_stream_writer)를 추가하면 LangGraph 실행 컨텍스트 외부에서 tool을 호출할 수 없습니다.
</Note>
...
이는 [multi-agent](/oss/python/langchain/multi-agent) 시스템에서 어떤 agent가 출력을 streaming할지 제어하는 데 유용합니다.

Streaming을 비활성화하는 방법을 알아보려면 [Models](/oss/python/langchain/models#disable-streaming) 가이드를 참조하세요.
