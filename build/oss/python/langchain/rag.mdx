...
LLM이 가능하게 하는 가장 강력한 애플리케이션 중 하나는 정교한 질의응답(Q&A) 챗봇입니다. 이는 특정 소스 정보에 대한 질문에 답변할 수 있는 애플리케이션입니다. 이러한 애플리케이션은 Retrieval Augmented Generation 또는 [RAG](/oss/python/langchain/retrieval/)로 알려진 기술을 사용합니다.
...
데이터를 인덱싱한 후에는 retrieval 및 generation 단계를 구현하기 위한 오케스트레이션 프레임워크로 [에이전트](/oss/python/langchain/agents)를 사용할 것입니다.

<Note>
    이 튜토리얼의 인덱싱 부분은 대부분 [semantic search 튜토리얼](/oss/python/langchain/knowledge-base)을 따릅니다.
...
<Note>
**이 섹션은 [semantic search 튜토리얼](/oss/python/langchain/knowledge-base)의 내용을 축약한 버전입니다.**

데이터가 이미 인덱싱되어 검색 가능한 경우(즉, 검색을 실행하는 함수가 있는 경우) 또는 [document loaders](/oss/python/langchain/retrieval#document_loaders), [embeddings](/oss/python/langchain/retrieval#embedding_models), [vector stores](/oss/python/langchain/retrieval#vectorstores)에 익숙한 경우 [retrieval and generation](/oss/python/langchain/rag#2-retrieval-and-generation)에 대한 다음 섹션으로 건너뛰어도 됩니다.

</Note>

인덱싱은 일반적으로 다음과 같이 작동합니다:

1. **Load**: 먼저 데이터를 로드해야 합니다. 이는 [Document Loaders](/oss/python/langchain/retrieval#document_loaders)로 수행됩니다.
2. **Split**: [Text splitters](/oss/python/langchain/retrieval#text_splitters)는 큰 `Documents`를 더 작은 청크로 나눕니다. 이는 데이터를 인덱싱하고 모델에 전달하는 데 유용합니다. 큰 청크는 검색하기 어렵고 모델의 제한된 context window에 맞지 않기 때문입니다.
3. **Store**: 나중에 검색할 수 있도록 분할된 데이터를 저장하고 인덱싱할 장소가 필요합니다. 이는 종종 [VectorStore](/oss/python/langchain/retrieval#vectorstores)와 [Embeddings](/oss/python/langchain/retrieval#embedding_models) 모델을 사용하여 수행됩니다.

![index_diagram](/images/rag_indexing.png)

### 문서 로드

먼저 블로그 게시물 내용을 로드해야 합니다. 이를 위해 [DocumentLoaders](/oss/python/langchain/retrieval#document_loaders)를 사용할 수 있습니다. 이는 소스에서 데이터를 로드하고 [Document](https://reference.langchain.com/python/langchain_core/documents/#langchain_core.documents.base.Document) 객체 목록을 반환하는 객체입니다.

이 경우 [`WebBaseLoader`](/oss/python/integrations/document_loaders/web_base)를 사용합니다. 이는 `urllib`을 사용하여 웹 URL에서 HTML을 로드하고 `BeautifulSoup`을 사용하여 텍스트로 파싱합니다. `bs_kwargs`를 통해 `BeautifulSoup` 파서에 매개변수를 전달하여 HTML -\> 텍스트 파싱을 사용자 정의할 수 있습니다([BeautifulSoup 문서](https://beautiful-soup-4.readthedocs.io/en/latest/#beautifulsoup) 참조). 이 경우 "post-content", "post-title" 또는 "post-header" 클래스가 있는 HTML 태그만 관련이 있으므로 다른 모든 태그를 제거합니다.
...
- [통합](/oss/python/integrations/document_loaders/): 선택할 수 있는 160개 이상의 통합.
- [`BaseLoader`](https://reference.langchain.com/python/langchain_core/document_loaders/#langchain_core.document_loaders.BaseLoader): 기본 인터페이스에 대한 API 참조.
...
이를 처리하기 위해 [`Document`](https://reference.langchain.com/python/langchain_core/documents/#langchain_core.documents.base.Document)를 embedding 및 vector 저장을 위한 청크로 분할합니다. 이렇게 하면 런타임에 블로그 게시물의 가장 관련성 높은 부분만 검색하는 데 도움이 됩니다.

[semantic search 튜토리얼](/oss/python/langchain/knowledge-base)에서와 같이 `RecursiveCharacterTextSplitter`를 사용합니다. 이는 각 청크가 적절한 크기가 될 때까지 새 줄과 같은 일반적인 구분 기호를 사용하여 문서를 재귀적으로 분할합니다. 이는 일반 텍스트 사용 사례에 권장되는 text splitter입니다.
...
`TextSplitter`: [`Document`](https://reference.langchain.com/python/langchain_core/documents/#langchain_core.documents.base.Document) 객체 목록을 저장 및 검색을 위한 더 작은 청크로 분할하는 객체.

- [통합](/oss/python/integrations/splitters/)
...
이제 런타임에 검색할 수 있도록 66개의 텍스트 청크를 인덱싱해야 합니다. [semantic search 튜토리얼](/oss/python/langchain/knowledge-base)에 따라 각 문서 분할의 내용을 [embed](/oss/python/langchain/retrieval#embedding_models/)하고 이러한 embedding을 [vector store](/oss/python/langchain/retrieval#vectorstores/)에 삽입하는 접근 방식을 사용합니다. 입력 쿼리가 주어지면 vector 검색을 사용하여 관련 문서를 검색할 수 있습니다.

[튜토리얼 시작 부분](/oss/python/langchain/rag#components)에서 선택한 vector store 및 embeddings model을 사용하여 단일 명령으로 모든 문서 분할을 embed하고 저장할 수 있습니다.
...
- [통합](/oss/python/integrations/text_embedding/): 선택할 수 있는 30개 이상의 통합.
- [인터페이스](https://reference.langchain.com/python/langchain_core/embeddings/#langchain_core.embeddings.embeddings.Embeddings): 기본 인터페이스에 대한 API 참조.

`VectorStore`: embedding을 저장하고 쿼리하는 데 사용되는 vector database의 래퍼.

- [통합](/oss/python/integrations/vectorstores/): 선택할 수 있는 40개 이상의 통합.
...
1. **Retrieve**: 사용자 입력이 주어지면 [Retriever](/oss/python/langchain/retrieval#retrievers)를 사용하여 저장소에서 관련 분할을 검색합니다.
2. **Generate**: [모델](/oss/python/langchain/models)이 질문과 검색된 데이터를 모두 포함하는 프롬프트를 사용하여 답변을 생성합니다.
...
RAG 애플리케이션의 한 가지 형태는 정보를 검색하는 tool이 있는 간단한 [에이전트](/oss/python/langchain/agents)입니다. vector store를 래핑하는 [tool](/oss/python/langchain/tools)을 구현하여 최소한의 RAG 에이전트를 조립할 수 있습니다:
...
여기서는 [tool decorator](https://reference.langchain.com/python/langchain/tools/#langchain.tools.tool)를 사용하여 각 [ToolMessage](/oss/python/langchain/messages#tool-message)에 원시 문서를 [artifacts](/oss/python/langchain/messages#param-artifact)로 첨부하도록 tool을 구성합니다. 이를 통해 모델로 전송되는 문자열화된 표현과 별도로 애플리케이션에서 문서 메타데이터에 액세스할 수 있습니다.
...
<Tip>
    [LangGraph](/oss/python/langgraph/overview) 프레임워크를 직접 사용하여 더 깊은 수준의 제어 및 사용자 정의를 추가할 수 있습니다. 예를 들어 문서 관련성을 평가하고 검색 쿼리를 다시 작성하는 단계를 추가할 수 있습니다. 더 고급 형태는 LangGraph의 [Agentic RAG 튜토리얼](/oss/python/langgraph/agentic-rag)을 확인하세요.
</Tip>


### RAG 체인

위의 [agentic RAG](#rag-agents) 형태에서는 LLM이 사용자 쿼리에 답변하는 데 도움이 되는 [tool call](/oss/python/langchain/models#tool-calling)을 생성하는 데 재량을 사용하도록 허용합니다. 이는 범용적으로 좋은 솔루션이지만 몇 가지 트레이드오프가 있습니다:
...
2. 해당 키를 채우기 위해 [pre-model hook](/oss/python/langchain/agents#pre-model-hook)을 통해 새 노드 추가(컨텍스트 주입도 함께)
...
이제 [`create_agent`](https://reference.langchain.com/python/langchain/agents/#langchain.agents.create_agent)를 통해 간단한 RAG 애플리케이션을 구현했으므로 새로운 기능을 쉽게 통합하고 더 깊이 들어갈 수 있습니다:

- 반응형 사용자 경험을 위해 token 및 기타 정보 [스트리밍](/oss/python/langchain/streaming)
- 다중 턴 상호 작용을 지원하기 위해 [대화 메모리](/oss/python/langchain/short-term-memory) 추가
- 대화 스레드 간 메모리를 지원하기 위해 [장기 메모리](/oss/python/langchain/long-term-memory) 추가
- [구조화된 응답](/oss/python/langchain/structured-output) 추가
