...
이 가이드는 LangSmith 애플리케이션에 커스텀 인증을 추가하는 방법을 보여줍니다. 이 페이지의 단계는 [클라우드](/langsmith/cloud) 및 [자체 호스팅](/langsmith/self-hosted) 배포 모두에 적용됩니다. 자체 커스텀 서버에서 [LangGraph 오픈 소스 라이브러리](/oss/python/langgraph/overview)를 독립적으로 사용하는 경우에는 적용되지 않습니다.
