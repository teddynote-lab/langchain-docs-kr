...
interrupt에 대한 자세한 내용은 [How to handle interrupts](/oss/python/langgraph/interrupts#pause-using-interrupt) 가이드를 참조하세요.
...
* `onCustomEvent`: 커스텀 이벤트가 수신될 때 호출됩니다. 커스텀 이벤트를 스트리밍하는 방법은 [streaming](/oss/python/langgraph/streaming#stream-custom-data) 가이드를 참조하세요.
