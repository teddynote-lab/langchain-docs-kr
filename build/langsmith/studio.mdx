...
* [time travel](/oss/python/langgraph/use-time-travel)을 통한 에이전트 state 디버깅
...
Chat mode는 채팅 전용 에이전트를 반복 작업하고 테스트하기 위한 더 간단한 UI입니다. 비즈니스 사용자와 전반적인 에이전트 동작을 테스트하려는 사용자에게 유용합니다. Chat mode는 state가 [`MessagesState`](/oss/python/langgraph/use-graph-api#messagesstate)를 포함하거나 확장하는 graph에서만 지원됩니다.
