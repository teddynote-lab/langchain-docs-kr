...
control plane과 [data plane](/langsmith/data-plane) "listener" application은 조정하여 각 배포에 대한 Postgres 데이터베이스를 자동으로 생성합니다. 데이터베이스는 배포의 [persistence layer](/oss/python/langgraph/persistence#memory-store) 역할을 합니다.

LangGraph application을 구현할 때 개발자가 [checkpointer](/oss/python/langgraph/persistence#checkpointer-libraries)를 구성할 필요가 없습니다. 대신 graph에 대해 checkpointer가 자동으로 구성됩니다. graph에 대해 구성된 모든 checkpointer는 자동으로 구성된 것으로 대체됩니다.
